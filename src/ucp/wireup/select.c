        ...
        ucp_wireup_select_context_t *select_ctx, int show_error)
{
    ...
    for (lane_desc = select_ctx->lane_descs;
        ...
        if ((lane_desc->rsc_index == select_info->rsc_index) &&
            ...
            ucs_assertv_always(
                    ucs_score_cmp(lane_desc->score[lane_type],
                                  select_info->score) == 0,
                                    ...
                                    void *arg, ucp_lane_type_t lane_type)
{
    ...
    /* reverse the value of ucs_score_cmp to sort scores in descending order */
    return -ucs_score_cmp(score1, score2);
}
        ...
        ucp_wireup_select_context_t *select_ctx)
{
    ...
    for (;;) {
        ...
        if ((status != UCS_OK) ||
            ...
            (ucs_score_cmp(select_info.score, reg_score) <= 0)) {
            break;
        }
                                    ...
                                    void *is_local)
{
    ...
    score_cmp = ucs_score_cmp(info1->score, info2->score);
    if (score_cmp != 0) {
        return -score_cmp;
    }
                        ...
                        ucp_wireup_select_context_t *select_ctx)
{
    ...
    status = ucp_wireup_select_transport(select_ctx, select_params, &criteria,
                                         ...
                                         &select_info);
    if ((status == UCS_OK) &&
        (ucs_score_cmp(select_info.score, am_info->score) >= 0)) {
