                    ...
                    const ucp_lane_index_t *lanes2remote)
{
    ...
    req->send.wireup.src_ep_key  = ucp_ep_ext_gen(ep)->lkey;
    if (ep->flags & UCP_EP_FLAG_DEST_EP) {
        req->send.wireup.dst_ep_key = ucp_ep_dest_ep_key(ep);
    } else {
        req->send.wireup.dst_ep_key = UCP_EP_HASH_INVALID_KEY;
    }
                               ...
                               const ucp_unpacked_address_t *remote_address)
{
    ...
    ucs_assert(msg->dst_ep_key != UCP_EP_HASH_INVALID_KEY);
    ucs_trace("got wireup pre_request from 0x%"PRIx64" src_ep_key 0x%lx dst_ep_key 0x%lx conn_sn %u",
              remote_address->uuid, msg->src_ep_key, msg->dst_ep_key,
              msg->conn_sn);

    /* wireup pre_request for a specific ep */
    ep = ucp_worker_get_ep_by_key(worker, msg->dst_ep_key);
    ucs_assert(ep->flags & UCP_EP_FLAG_SOCKADDR_PARTIAL_ADDR);

    ucp_ep_update_rkey(ep, msg->src_ep_key);
                           ...
                           const ucp_unpacked_address_t *remote_address)
{
    ...
    ucs_trace("got wireup request from 0x%"PRIx64" src_ep_key 0x%lx dst_ep_key 0x%lx conn_sn %d",
              remote_address->uuid, msg->src_ep_key, msg->dst_ep_key,
              msg->conn_sn);
    ...
    if (msg->dst_ep_key != UCP_EP_HASH_INVALID_KEY) {
     ...
     */
    send_reply = (msg->dst_ep_key == UCP_EP_HASH_INVALID_KEY) ||
                         ...
                         const ucp_unpacked_address_t *remote_address)
{
    ...
    ucs_trace("ep %p: got wireup reply src_ep_key 0x%lx dst_ep)key 0x%lx sn %d",
              ep, msg->src_ep_key, msg->dst_ep_key, msg->conn_sn);
                                ...
                                char *buffer, size_t max)
{
    ...
    snprintf(p, end - p,
             ...
             unpacked_address.uuid, msg->src_ep_key, msg->dst_ep_key,
             msg->conn_sn);
    p += strlen(p);
