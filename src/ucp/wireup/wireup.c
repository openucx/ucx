                       ...
                       size_t *address_length_p)
{
    ...
    if (is_rank_update) {
        msg_hdr->score = ucp_wireup_get_ep_usage_score(ep) /
...
ucp_wireup_send_rank_update(ucp_ep_h ep, uint8_t type, int is_external_event)
{
    ...
    if ((ep->flags & UCP_EP_FLAG_CONNECT_PRE_REQ_QUEUED) ||
...
{
    ucp_wireup_send_rank_update(entry, UCP_WIREUP_MSG_PROMOTE,
                                is_external_event);
}
...
void ucp_wireup_send_demotion_request(void *entry, void *arg,
                                      int is_external_event)
{
    ucp_wireup_send_rank_update(entry, UCP_WIREUP_MSG_DEMOTE,
                                is_external_event);
}
                               ...
                               const ucp_unpacked_address_t *remote_address)
{
    ...
    ucp_wireup_process_pre_flow_msg(ep, remote_address, ep_init_flags,
                                    msg->src_ep_id);
}
        ...
        const ucp_unpacked_address_t *remote_address)
{
    ...
    if (ep == NULL) {
        if (kh_size(&usage_tracker->hash) >= usage_tracker->params.promote_thresh) {
            return;
        }
