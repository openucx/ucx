...
static void ucp_wireup_send_pre_flow_msg(ucp_ep_h ep, uint8_t type)
{
    ucs_status_t status;
...
static void
ucp_wireup_send_rank_update(ucp_ep_h ep, uint8_t type, int is_external_event)
{
...
static int
ucp_wireup_should_ignore_request(ucp_ep_h ep, uint64_t remote_uuid,
                                 uint8_t msg_type)
{
        ...
        const ucp_unpacked_address_t *remote_address)
{
    ...
    ucs_debug("ep %p: demotion request from 0x%.8lX accepted (score: %.2f)",
              ep, msg->src_ep_id, msg->score * UCP_WIREUP_MSG_SCORE_TO_DOUBLE);
...
static int ucp_wireup_has_p2p_lanes(ucp_ep_h ep, const ucp_ep_config_key_t *key)
{
    ucp_lane_index_t lane;

    for (lane = 0; lane < key->num_lanes; ++lane) {
        if (ucp_ep_config_connect_p2p(ep->worker, key,
                                  ...
                                  int *am_need_flush_p)
{
    ...
    *am_need_flush_p     = 0;
    ...
    if (!ucp_wireup_should_reconfigure(ep, reuse_lane_map,
                                       new_key->num_lanes)) {
        /* Restore wireup lane and connection bitmap */
    ...
    } else if (ucp_wireup_has_p2p_lanes(ep, new_key)) {
        /* Old wireup lane won't be reused, yet wireup is required for p2p
         * lanes. Previous wireup lane is not part of new configuration, so add
         * it as auxiliary endpoint inside CM/non-reused lane, to be able to
...
void ucp_wireup_send_pre_request(ucp_ep_h ep)
{
    ...
    ucp_wireup_send_pre_flow_msg(ep, UCP_WIREUP_MSG_PRE_REQUEST);
}
