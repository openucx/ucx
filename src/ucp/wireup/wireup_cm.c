...
ucp_cm_ep_priv_data_pack(ucp_ep_h ep, const ucp_tl_bitmap_t *tl_bitmap,
                         ...
                         uint8_t addr_mode, int priv_data_pack)
{
    ucp_worker_h worker = ep->worker;
    void *ucp_addr      = NULL;
    ...
    if (worker->cms[cm_idx].attr.max_conn_priv < priv_data_length) {
    ...
    sa_data = ucs_malloc(priv_data_length, "client_priv_data");
    if (sa_data == NULL) {
    ...
    *data_buf_p        = sa_data;
    *data_buf_length_p = priv_data_length;
    status             = UCS_OK;
...
initial_config_retry:
    ...
    if (ep_init_flags & UCP_EP_INIT_CREATE_AM_LANE_ONLY) {
        ...
        address_mode = UCP_WIREUP_SA_DATA_CM_ADDR;
    }
...
static unsigned ucp_cm_client_connect_progress(void *arg)
{
    ...
    if (progress_arg->sa_data->addr_mode == UCP_WIREUP_SA_DATA_AM_ONLY) {
