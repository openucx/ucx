...
typedef struct {
    ...
    /* packed rkeys follow */
} UCS_S_PACKED ucp_am_rdma_rtr_hdr_t;
...
typedef struct {
    uintptr_t                 rreq_ptr; /* request on the rndv receiver side */
    size_t                    offset;
} UCS_S_PACKED ucp_am_rdma_data_hdr_t;


ucs_status_t ucp_am_rdma_send_start_rndv(ucp_request_t *req);
...
//                                  unsigned tl_flags);
//
size_t ucp_am_rdma_rts_pack(void *dest, void *arg);
