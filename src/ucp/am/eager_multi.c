...
static int ucp_am_eager_multi_zcopy_hw_token_available(
        const ucp_proto_init_params_t *init_params)
{
    ucp_lane_index_t am_lane = init_params->ep_config_key->am_lane;
    ...
    iface_attr = ucp_proto_common_get_iface_attr(init_params, am_lane);
    return !!(iface_attr->cap.flags & UCT_IFACE_FLAG_EP_TOKEN_QUERY);
}

static void ucp_am_eager_multi_zcopy_hw_token_proto_probe(
        const ucp_proto_init_params_t *init_params)
{
    if (init_params->ep_config_key->dst_version < 21) {
        return;
    ...
    if (!ucp_am_eager_multi_zcopy_hw_token_available(init_params)) {
        return;
    }
...
static UCS_F_ALWAYS_INLINE ucs_status_t
ucp_am_eager_multi_zcopy_hw_token_send_func(
        ucp_request_t *req, const ucp_proto_multi_lane_priv_t *lpriv,
        ucp_datatype_iter_t *next_iter, ucp_lane_index_t *lane_shift)
{
    /* Query opaque token right before sending the first fragment so it
     * reflects the actual transport state. Querying at init time would be
     * stale if the send queue is filled and no work was posted. */
    if (ucp_proto_am_is_first_fragment(req)) {
        uct_ep_attr_t ep_attr;
        ucs_status_t status;
        uct_ep_h uct_ep;

        uct_ep = ucp_ep_get_lane(req->send.ep, lpriv->super.lane);
        ...
        req->send.msg_proto.message_id = ep_attr.tx_token;
    }

    return ucp_am_eager_multi_zcopy_send_func_common(req, lpriv, next_iter,
                                                     lane_shift,
                                                     UCP_AM_ID_AM_FIRST_HW_TOKEN,
                                                     UCP_AM_ID_AM_MIDDLE_HW_TOKEN);
}

static ucs_status_t ucp_am_eager_multi_zcopy_hw_token_init(ucp_request_t *req)
{
    ucp_am_eager_zcopy_pack_user_header(req);
    return UCS_OK;
}
...
static ucs_status_t
ucp_am_eager_multi_zcopy_hw_token_proto_progress(uct_pending_req_t *self)
{
    ucp_request_t *req = ucs_container_of(self, ucp_request_t, send.uct);
    ucs_status_t status;
    ...
    status = ucp_proto_multi_zcopy_progress(
            ...
            ucp_am_eager_multi_zcopy_hw_token_init,
            UCT_MD_MEM_ACCESS_LOCAL_READ, UCP_DT_MASK_CONTIG_IOV,
            ucp_am_eager_multi_zcopy_hw_token_send_func,
            ucp_request_invoke_uct_completion_success,
            ucp_am_eager_zcopy_completion);
...
ucp_proto_t ucp_am_eager_multi_zcopy_hw_token_proto = {
    .name     = "am/egr/multi/zcopy/hw_token",
    .desc     = UCP_PROTO_MULTI_FRAG_DESC " " UCP_PROTO_ZCOPY_DESC " hw_token",
    .flags    = 0,
    .probe    = ucp_am_eager_multi_zcopy_hw_token_proto_probe,
    .query    = ucp_proto_multi_query,
    .progress = {ucp_am_eager_multi_zcopy_hw_token_proto_progress},
    .abort    = ucp_proto_am_request_zcopy_abort,
    .reset    = ucp_am_proto_request_zcopy_reset
};
