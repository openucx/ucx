        ...
        size_t min_length, size_t max_length)
{
    ...
    do {
        ...
        ucs_trace("%s: max %zu remote-op %s %s" UCP_PROTO_PERF_FUNC_TYPES_FMT,
                  params->super.super.proto_name, remote_range->max_length, rndv_op_name,
                                  ...
                                  ucp_proto_config_t *cfg)
{
    ...
    cfg->rkey_cfg_index = init_params->rkey_cfg_index;
    cfg->select_param   = *select_param;
}
                               ...
                               ucp_lane_index_t lane)
{
    ...
    ucs_array_for_each(remote_proto, &remote_proto_init.protocols) {
        ...
        if ((remote_proto->cfg_thresh != UCS_MEMUNITS_AUTO)) {
            ...
            /* If some remote variant has certain thresh, we need to
             * increase their cfg_priority to force it in case of the same
             * params->super.cfg_thresh and remote_proto->cfg_thresh.
             *    E.g. RNDV_THRESH=0 and RNDV_MODE=get_zcopy
             *    In this case all the variants would have 0 cfg_thresh, but
             *    get_zcopy should be preferred over other RNDV protocols.
             * 
             * +1 is required in case if remote variant cfg_priority is 0.
             */
            if (remote_proto->cfg_thresh != UCS_MEMUNITS_INF) {
                cfg_priority += remote_proto->cfg_priority + 1;
            }
        }
...
out:
    if (status != UCS_OK) {
        ucs_debug("error during probing %s, status \"%s\"",
                  params->ctrl_msg_name, ucs_status_string(status));
    }
}
...
ucp_proto_rndv_ctrl_am_probe(const ucp_proto_rndv_ctrl_init_params_t *params)
{
    ...
    ucp_proto_rndv_ctrl_probe(params, lane);
}
