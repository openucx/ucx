...
/* Sys device is crucial for rkey_unpack in rndv pipeline protocols that use
 * CUDA staging buffers. If sys_dev of the user memory is unknown (e.g. host
 * or CUDA-managed memory), use the default sys_dev since it will be used for
 * allocating staging buffers anyway. */
static UCS_F_ALWAYS_INLINE ucs_sys_device_t
ucp_proto_rndv_sys_dev(ucp_context_h context, ucs_memory_type_t mem_type,
                 ...
                 const void *rkey_buffer, size_t rkey_length, uint8_t sg_count)
{
    ...
    if (rkey_length > 0) {
        sys_dev = ucp_proto_rndv_sys_dev(
