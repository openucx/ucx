...
ucp_proto_multi_filter_slow_lanes(const ucp_lane_index_t *lanes,
                                  ...
                                  double max_bandwidth, double max_bw_ratio)
{
...
ucp_proto_multi_init_priv(const ucp_proto_multi_init_params_t *params,
                          ucp_proto_common_tl_perf_t *lanes_perf,
                          ucp_lane_map_t lane_map, ucp_md_map_t reg_md_map,
                          double max_frag_ratio, size_t min_rndv_chunk_size,
                          ...
                          ucp_proto_multi_priv_t *mpriv)
{
    ...
    ucs_for_each_bit(lane, lane_map) {
        ...
        */
        ...
        acc_perf->min_length = ucs_max(acc_perf->min_length, min_length);

        weight_sum           += lpriv->weight;
        min_end_offset       += lane_perf->bandwidth * min_rndv_chunk_size /
                                min_bandwidth;
                                 ...
                                 ucp_proto_perf_t *perf)
{
    ...
    ucs_assertv((params->multi_lane_thresh != SIZE_MAX) &&
                (params->multi_lane_thresh >= single_lane_params.min_length) &&
                (params->multi_lane_thresh <= single_lane_params.max_length),
                "multi_lane_thresh=%zu min_length=%zu max_length=%zu",
                params->multi_lane_thresh, single_lane_params.min_length,
                single_lane_params.max_length);

    single_lane_params.max_length = params->multi_lane_thresh;
    return ucp_proto_init_perf(&single_lane_params, lane_perf, perf_node,
                               reg_md_map, perf_name, perf);
}
                                  ...
                                  ucp_proto_multi_priv_t *mpriv)
{
    ...
    const double min_rndv_chunk_size = context->config.ext.min_rndv_chunk_size;
    ...
    /* Select the lanes to use */
    lane_map = ucp_proto_multi_filter_slow_lanes(lanes, lanes_perf, num_lanes,
                                                 params->max_lanes,
                                                 max_bandwidth, max_bw_ratio);

    /* Calculate lanes aggregated performance */
    perf = ucp_proto_multi_accumulate_perf(lanes, lanes_perf, lane_map,
                                           &max_frag_ratio);

    /* Initialize multi-lane private data and relative weights */
    reg_md_map = ucp_proto_common_reg_md_map(&params->super, lane_map);

    /* Initialize priv buffer and set `max_frag` and `min_length` for perf */
    ucp_proto_multi_init_priv(params, lanes_perf, lane_map, reg_md_map,
                              max_frag_ratio, min_rndv_chunk_size,
                              min_bandwidth, &perf, mpriv);

    status = ucp_proto_perf_create(perf_name, perf_p);
    if (status != UCS_OK) {
        return status;
    }
    ...
    if (mpriv->num_lanes == 1) {
        ...
        /* Report 1 lane perf until `multi_lane_thresh` */
        lane   = ucs_ffs64(lane_map);
        status = ucp_proto_multi_single_lane_perf(params, &lanes_perf[lane],
                                                  lanes_perf_nodes[lane],
                                                  reg_md_map, perf_name,
                                                  *perf_p);
        if (status != UCS_OK) {
            goto err_cleanup_perf;
        }

        /* Prepare multi-lane perf to be initialized */
        ucs_assertv(params->multi_lane_thresh >= perf.min_length,
                    "multi_lane_thresh=%zu min_length=%zu",
                    params->multi_lane_thresh, perf.min_length);
        perf.min_length = params->multi_lane_thresh + 1;
