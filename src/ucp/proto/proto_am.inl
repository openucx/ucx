                                   ...
                                   int enable_am_bw)
{
    ...
    if (offset == 0) {
        ...
        return UCS_STATUS_IS_ERR(packed_len) ? packed_len : UCS_OK;
    }

    /* TODO: handle NO_RESOURCE error & switch to next available
     * lane if possible */
    return UCS_STATUS_IS_ERR(packed_len) ? packed_len : UCS_INPROGRESS;
}
