...
static ucs_status_t ucp_worker_add_resource_ifaces(ucp_worker_h worker)
{
    ...
    if (!(UCS_BITMAP_IS_ZERO(ctx_tl_bitmap))) {
    ...
    if (UCS_BITMAP_IS_ZERO(ctx_tl_bitmap)) {
        ...
        ucs_debug("selected tl bitmap: 0x%" PRIx64 " 0x%" PRIx64 " (%zu tls)",
                  tl_bitmap.bits[0], tl_bitmap.bits[1],
    ...
    ucs_debug("selected scalable tl bitmap: 0x%"PRIx64" 0x%"PRIx64" (%zu tls)",
              worker->scalable_tl_bitmap.bits[0], worker->scalable_tl_bitmap.bits[1],
