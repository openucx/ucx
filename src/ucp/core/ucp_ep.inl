...
static inline void ucp_ep_flush_state_reset(ucp_ep_h ep)
{
    ...
    if (!(ep->flags & UCP_EP_FLAG_FLUSH_STATE_VALID)) {
        flush_state->sw.send_sn = 0;
        flush_state->sw.cmpl_sn = 0;
        ucs_queue_head_init(&flush_state->sw.reqs);
        ep->flags |= UCP_EP_FLAG_FLUSH_STATE_VALID;
    } else {
        ucs_assert(flush_state->sw.send_sn == 0);
        ucs_assert(flush_state->sw.cmpl_sn == 0);
        ucs_assert(ucs_queue_is_empty(&flush_state->sw.reqs));
    }
}
