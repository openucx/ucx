                                 ...
                                 ucp_lane_map_t *failover_lanes_p)
{
    ...
    if (failover_lanes_p != NULL) {
        status = ucp_ep_failover_add_lanes(ep, lanes, uct_eps, discard_status,
                         ...
                         ucs_status_t discard_status)
{
    ...
    ucp_ep_discard_lanes(ucp_ep, failed_lanes, discard_status, old_cfg_index,
                         &failover_lanes);
    if (failover_lanes != 0) {
        status = ucp_ep_failover_query_lanes(ucp_ep, failover_lanes);
        if (status != UCS_OK) {
            ucs_debug("ep %p: aborting failover for lanes 0x%lx: failed to send "
                      "lane_state query: %s",
                      ucp_ep, failover_lanes, ucs_status_string(status));
            ucp_ep_failover_abort_lanes(ucp_ep, failover_lanes, status);
        }
    }
