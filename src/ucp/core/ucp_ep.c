                                 ...
                                 const ucp_ep_params_t *params, ucp_ep_h *ep_p)
{
     ...
     * in ep_match.
     */
    conn_sn = ucp_worker_ep_match_get_sn(worker, remote_address.uuid);
    ep      = ucp_worker_ep_match_retrieve(worker, remote_address.uuid,
                                           conn_sn ^
    ...
    if ((remote_address.uuid == worker->uuid) &&
        ...
        ucp_worker_ep_match_insert(worker, ep, remote_address.uuid,
                                   conn_sn, 1);
    }
...
void ucp_ep_disconnected(ucp_ep_h ep, int force)
{
    ...
    ucp_worker_ep_match_remove_ep(ep->worker, ep);
    ucp_ep_destroy_internal(ep);
}
