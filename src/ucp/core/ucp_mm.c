...
static void ucp_memh_put_parent(ucp_context_h context, ucp_mem_h memh)
{
    ucs_rcache_t *rcache;
    khiter_t iter;
...
static void ucp_memh_cleanup(ucp_context_h context, ucp_mem_h memh)
{
    ...
    if (memh->parent != NULL) {
        ...
        UCP_THREAD_CS_ENTER(&context->mt_lock);
        ucp_memh_put_parent(context, memh->parent);
        UCP_THREAD_CS_EXIT(&context->mt_lock);
    } else {
...
ucs_status_t ucp_memh_register_slow(ucp_context_h context, ucp_mem_h memh,
                                    ucp_md_map_t md_map, unsigned uct_flags)
{
    ...
    if (context->config.ext.reg_nb_mem_types & UCS_BIT(memh->mem_type)) {
    ...
    ucs_for_each_bit(md_index, md_map) {
        ...
        if (ucs_unlikely(status != UCS_OK)) {
            ucp_memh_register_log_fail(err_level, address, length,
                                       memh->mem_type, reg_params.dmabuf_fd,
                                       md_index, context, status);
            if (uct_flags & UCT_MD_MEM_FLAG_HIDE_ERRORS) {
                continue;
            }
...
out_zero_mem:
    ...
    memh   = ucp_memh_get_zero_mem(context);
    status = UCS_OK;
    goto out;
}
...
ucs_status_t ucp_mem_unmap(ucp_context_h context, ucp_mem_h memh)
{
    ...
    ucs_assert(memh->super.refcount == 1);
    ucp_memh_cleanup(context, memh);
    ucs_free(memh);
    return UCS_OK;
}
...
ucp_mpool_free(ucp_worker_h worker, ucs_mpool_t *mp, void *chunk)
{
    ...
    ucp_mem_unmap(worker->context, chunk_hdr->memh);
}
...
ucp_rndv_frag_free_mpools(ucs_mpool_t *mp, void *chunk)
{
    ...
    /* coverity[check_return] */
    ucp_mem_unmap(mpriv->worker->context, chunk_hdr->memh);
    ucs_free(chunk_hdr);
}
...
ucp_mm_get_alloc_md_index(ucp_context_h context, ucp_md_index_t *md_idx)
{
    ...
    if (!context->alloc_md_index_initialized) {
        ...
        /* coverity[check_return] */
        ucp_mem_unmap(context, alloc_memh);
    }
