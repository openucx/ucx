...
static ucs_status_t
ucp_mem_alloc(ucp_context_h context, void *address, size_t length,
              unsigned uct_flags, ucs_memory_type_t mem_type, const char *name,
...
static void ucp_memh_release(ucp_context_h context,
                             ucp_mem_rcache_region_t *region,
                             ucp_md_map_t md_map)
{
    ...
    ucs_for_each_bit(md_index, md_map) {
        ...
        if (md_index == region->memh.alloc_md_index) {
            continue;
        }

        ucs_trace("de-registering memh[%d]=%p", md_index,
                  region->memh.uct[md_index]);
        status = uct_md_mem_dereg(context->tl_mds[md_index].md,
                                  region->memh.uct[md_index]);
        if (status != UCS_OK) {
            ucs_warn("failed to dereg from md[%d]=%s: %s", md_index,
...
static ucs_status_t
ucp_memh_get_common(ucp_context_h context, void *address, size_t length,
                    unsigned uct_flags, ucs_memory_type_t mem_type,
                    ucp_md_map_t reg_md_map, ucp_mem_h *memh_p, int alloc,
                    const char *alloc_name)
{
    ...
    void *reg_address;
    size_t reg_length;
    ucp_md_map_t new_md_map, new_md_map_ok;
    ucs_status_t status;
    ucs_memory_info_t mem_info;
    ucs_log_level_t log_level;

    if (alloc) {
        status = ucp_mem_alloc(context, address, length, uct_flags, mem_type,
                               alloc_name, &mem);
        if (status != UCS_OK) {
            return status;
        }

        reg_address = mem.address;
        reg_length  = mem.length;
    } else if (context->config.ext.reg_whole_alloc_bitmap & UCS_BIT(mem_type)) {
        ucp_memory_detect_internal(context, address, length, &mem_info);
    ...
    if (context->rcache != NULL) {
        ...
        region = ucs_derived_of(rregion, ucp_mem_rcache_region_t);

        reg_address = (void*)rregion->super.start;
        reg_length  = rregion->super.end - rregion->super.start;
    } else {
        region = ucs_calloc(1, sizeof(ucp_mem_rcache_region_t) +
                            sizeof(uct_mem_h) * context->num_mds, "ucp rcache");
        if (!region) {
            return UCS_ERR_NO_MEMORY;
        }

        region->super.super.start   = (intptr_t)reg_address;
        region->super.super.end     = (intptr_t)reg_address + reg_length;
        region->memh.alloc_md_index = UCP_NULL_RESOURCE;
        region->memh.alloc_method   = UCT_ALLOC_METHOD_LAST;
    }
        ...
        if (mem.method == UCT_ALLOC_METHOD_MD) {
            for (md_index = 0; md_index < context->num_mds; md_index++) {
                if (mem.md == context->tl_mds[md_index].md) {
                    region->memh.alloc_md_index = md_index;
                    region->memh.uct[md_index]  = mem.memh;
                    region->memh.md_map        |= UCS_BIT(md_index);
                    break;
                }
            }
        }

        region->memh.alloc_method = mem.method;
        region->memh.mem_type     = mem.mem_type;
    } else {
        region->memh.mem_type     = mem_type;
    }

    new_md_map = ~region->memh.md_map & reg_md_map;
    new_md_map_ok = 0;
    ucs_for_each_bit(md_index, new_md_map) {
        ...
        if (ucs_unlikely(status != UCS_OK)) {
            ...
            ucs_log(log_level,
                    ...
                    ucs_status_string(status));
            ucp_memh_release(context, region, new_md_map_ok);
            ...
            if (context->rcache != NULL) {
                ucs_rcache_region_put(context->rcache, &region->super);
            } else {
                ucs_free(region);
            }
        ...
        ucs_trace("registered address %p length %zu on md[%d]=%s %p",
                  ...
                  context->tl_mds[md_index].rsc.md_name,
                  region->memh.uct[md_index]);
        new_md_map_ok |= UCS_BIT(md_index);
    }

    region->memh.md_map |= new_md_map_ok;
    *memh_p = &region->memh;
    return UCS_OK;
}

static void ucp_memh_put_common(ucp_context_h context, ucp_mem_h memh, int inv)
{
    ucp_mem_rcache_region_t *region = ucs_container_of(memh,
                                                       ucp_mem_rcache_region_t,
                                                       memh);
    uct_allocated_memory_t mem = {};
    ucs_status_t status;
    ...
    if (context->rcache != NULL) {
        if (inv) {
            ucs_rcache_region_invalidate(context->rcache, &region->super,
                                         ucs_empty_function, region);
        }
        ucs_rcache_region_put(context->rcache, &region->super);
    } else {
        ucp_memh_release(context, region, region->memh.md_map);
        ucs_free(region);
    }
}
...
{
    return ucp_memh_get_common(context, address, length, uct_flags, memory_type,
                               context->reg_md_map[memory_type], memh_p,
                               is_allocate, alloc_name);
}

int ucp_memh_zero_length(const ucp_mem_h memh) {
    return memh == &ucp_mem_dummy_handle;
}
...
{
    UCP_THREAD_CS_ENTER(&context->mt_lock);
    ucp_memh_put_common(context, memh, 1);
    UCP_THREAD_CS_EXIT(&context->mt_lock);
    return UCS_OK;
}
...
ucp_mpool_free(ucp_worker_h worker, ucs_mpool_t *mp, void *chunk)
{
    ...
    ucp_memh_put_common(worker->context, chunk_hdr->memh, 0);
}
...
ucp_rndv_frag_free_mpools(ucs_mpool_t *mp, void *chunk)
{
    ...
    ucp_memh_put_common(mpriv->worker->context, chunk_hdr->memh, 0);
    ucs_free(chunk_hdr);
}
...
ucp_mm_get_alloc_md_map(ucp_context_h context, ucp_md_map_t *md_map_p)
{
    ...
    if (!context->alloc_md_map_initialized) {
        ...
        ucp_memh_put_common(context, memh, 0);
    }
...
ucs_status_t ucp_memh_get(ucp_context_h context, void *address, size_t length,
                          unsigned uct_flags, ucs_memory_type_t mem_type,
                          ucp_md_map_t reg_md_map, ucp_mem_h *memh_p)
{
    if (length == 0) {
        *memh_p = &ucp_mem_dummy_handle;
        return UCS_OK;
    ...
    return ucp_memh_get_common(context, address, length, uct_flags, mem_type,
                               reg_md_map, memh_p, 0, "memh_get");
}

void ucp_memh_put(ucp_context_h context, ucp_mem_h memh)
{
    ucp_memh_put_common(context, memh, 0);
}
...
{
    ucp_mem_rcache_region_t *region = ucs_derived_of(rregion, ucp_mem_rcache_region_t);
    region->memh.md_map         = 0;
    region->memh.alloc_md_index = UCP_NULL_RESOURCE;
    region->memh.alloc_method   = UCT_ALLOC_METHOD_LAST;
...
static void ucp_mem_rcache_mem_dereg_cb(void *context, ucs_rcache_t *rcache,
                                       ucs_rcache_region_t *rregion)
{
    ucp_mem_rcache_region_t *region = ucs_derived_of(rregion, ucp_mem_rcache_region_t);
    ucp_memh_release(context, region, region->memh.md_map);
                                         ...
                                         size_t max)
{
    ...
    ucs_for_each_bit(md_index, region->memh.md_map) {
        off += snprintf(buf + off, max - off, " md[%d]=%s", md_index,
                        context->tl_mds[md_index].rsc.md_name);
    }
}
...
ucs_status_t ucp_mem_rcache_init(ucp_context_h context)
{
    ...
    rcache_params.region_struct_size = sizeof(ucp_mem_rcache_region_t) +
                                       sizeof(uct_mem_h) * context->num_mds;
    rcache_params.max_alignment      = ucs_get_page_size();
    rcache_params.max_unreleased     = 512 * UCS_MBYTE;
    ...
    status = ucs_rcache_create(&rcache_params, "ucp rcache",
                               ucs_stats_get_root(), &context->rcache);
    if (status != UCS_OK) {
        ucs_debug("failed to create registration cache: %s",
                  ucs_status_string(status));
    }
