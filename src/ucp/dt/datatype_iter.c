                                           ...
                                           unsigned uct_flags)
{
    ...
    memh = ucs_calloc(iov_count, sizeof(*memh), "dt_iov_memh");
    if (memh == NULL) {
    ...
    for (iov_index = 0; iov_index < iov_count; ++iov_index) {
        ...
        status = ucp_memh_get(context, iov->buffer, iov->length, uct_flags,
                              dt_iter->mem_info.type, md_map,
                              &memh[iov_index]);
        if (status != UCS_OK) {
