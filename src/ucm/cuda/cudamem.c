...
static ucs_memory_type_t ucm_cuda_get_mem_type(void *ptr)
{
    ...
    if (cu_err != cudaSuccess) {
        ucm_warn("cudaPointerGetAttributes(ptr=%p) failed: %s",
                 ptr, cudaGetErrorString(cu_err));
        return UCS_MEMORY_TYPE_CUDA;
    }

#if defined(HAVE_STRUCT_CUDAPOINTERATTRIBUTES_TYPE)
    ucs_assert((ptr_attrs.type == cudaMemoryTypeManaged) ||
               (ptr_attrs.type == cudaMemoryTypeDevice));
