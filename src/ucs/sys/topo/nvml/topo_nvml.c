...
static void
ucs_topo_get_memory_distance_nvml(ucs_sys_device_t device,
                                  ucs_sys_dev_distance_t *distance)
{
}

static ucs_sys_topo_provider_t ucs_sys_topo_provider_nvml = {
    .name = "nvml",
    .ops = {
        .get_distance        = ucs_topo_get_distance_nvml,
        .get_memory_distance = ucs_topo_get_memory_distance_nvml,
    }
};
...
UCS_STATIC_INIT
{
    ucs_list_add_tail(&ucs_sys_topo_providers_list,
                      &ucs_sys_topo_provider_nvml.list);
    nvmlInit_v2();
}

UCS_STATIC_CLEANUP {
    ucs_list_del(&ucs_sys_topo_provider_nvml.list);
    nvmlShutdown();
}
