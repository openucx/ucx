...
#define RISCV64_CPUID_GENUINEINTEL    "GenuntelineI" /* GenuineIntel in magic notation */
#define RISCV64_CPUID_AUTHENTICAMD    "AuthcAMDenti" /* AuthenticAMD in magic notation */
#define RISCV64_CPUID_CENTAURHAULS    "CentaulsaurH" /* CentaurHauls in magic notation */
#define RISCV64_CPUID_SHANGHAI        "  Shai  angh" /* Shanghai in magic notation */
#define RISCV64_CPUID_GET_MODEL       0x00000001u
#define RISCV64_CPUID_GET_BASE_VALUE  0x00000000u
#define RISCV64_CPUID_GET_EXTD_VALUE  0x00000007u
#define RISCV64_CPUID_GET_MAX_VALUE   0x80000000u
#define RISCV64_CPUID_INVARIANT_TSC   0x80000007u
#define RISCV64_CPUID_GET_CACHE_INFO  0x00000002u
#define RISCV64_CPUID_GET_LEAF4_INFO  0x00000004u

#define RISCV64_CPU_CACHE_RESERVED    0x80000000
#define RISCV64_CPU_CACHE_TAG_L1_ONLY 0x40
#define RISCV64_CPU_CACHE_TAG_LEAF4   0xff
...
static int ucs_x86_invariant_tsc()
{
    ...
    ucs_x86_cpuid(RISCV64_CPUID_GET_MAX_VALUE, &_eax, &_ebx, &_ecx, &_edx);
    if (_eax <= RISCV64_CPUID_INVARIANT_TSC) {
        goto warn;
    }

    ucs_x86_cpuid(RISCV64_CPUID_INVARIANT_TSC, &_eax, &_ebx, &_ecx, &_edx);
    if (!(_edx & UCS_BIT(8))) {
        goto warn;
    }
...
ucs_cpu_model_t ucs_arch_get_cpu_model()
{
    ...
    ucs_x86_cpuid(RISCV64_CPUID_GET_MODEL, ucs_unaligned_ptr(&version.reg), &_ebx, &_ecx, &_edx);
...
int ucs_arch_get_cpu_flag()
{
    ...
    if (UCS_CPU_FLAG_UNKNOWN == cpu_flag) {
        ...
        ucs_x86_cpuid(RISCV64_CPUID_GET_BASE_VALUE, &_eax, &_ebx, &_ecx, &_edx);
        base_value = _eax;

        if (base_value >= 1) {
            ucs_x86_cpuid(RISCV64_CPUID_GET_MODEL, &_eax, &_ebx, &_ecx, &_edx);
            if (_edx & (1 << 15)) {
        ...
        if (base_value >= 7) {
            ucs_x86_cpuid(RISCV64_CPUID_GET_EXTD_VALUE, &_eax, &_ebx, &_ecx, &_edx);
...
ucs_cpu_vendor_t ucs_arch_get_cpu_vendor()
{
    ...
    ucs_x86_cpuid(RISCV64_CPUID_GET_BASE_VALUE,
                  ucs_unaligned_ptr(&reg.eax), ucs_unaligned_ptr(&reg.ebx),
                  ucs_unaligned_ptr(&reg.ecx), ucs_unaligned_ptr(&reg.edx));
    if (!memcmp(reg.id, RISCV64_CPUID_GENUINEINTEL, sizeof(X86_CPUID_GENUINEINTEL) - 1)) {
        return UCS_CPU_VENDOR_INTEL;
    } else if (!memcmp(reg.id, RISCV64_CPUID_AUTHENTICAMD, sizeof(X86_CPUID_AUTHENTICAMD) - 1)) {
        return UCS_CPU_VENDOR_AMD;
    } else if (!memcmp(reg.id, RISCV64_CPUID_CENTAURHAULS, sizeof(X86_CPUID_CENTAURHAULS) - 1) ||
               !memcmp(reg.id, RISCV64_CPUID_SHANGHAI, sizeof(X86_CPUID_SHANGHAI) - 1)) {
...
ucs_status_t ucs_arch_get_cache_size(size_t *cache_sizes)
{
    ...
    ucs_x86_cpuid(RISCV64_CPUID_GET_BASE_VALUE,
                  ucs_unaligned_ptr(&reg.eax), ucs_unaligned_ptr(&reg.ebx),
                  ucs_unaligned_ptr(&reg.ecx), ucs_unaligned_ptr(&reg.edx));
    if (reg.eax < RISCV64_CPUID_GET_CACHE_INFO) {
        return UCS_ERR_UNSUPPORTED;
    }
    ...
    for (i = 0, max_iter = 1; i < max_iter; i++) {
        ucs_x86_cpuid(RISCV64_CPUID_GET_CACHE_INFO,
                      ucs_unaligned_ptr(&reg.eax), ucs_unaligned_ptr(&reg.ebx),
                      ucs_unaligned_ptr(&reg.ecx), ucs_unaligned_ptr(&reg.edx));
        ...
        for (r = 0; r < ucs_static_array_size(reg.reg); r++) {
            if (ucs_test_all_flags(reg.reg[r].value, RISCV64_CPU_CACHE_RESERVED)) {
                continue;
            }
            ...
            for (t = 0; (t < ucs_static_array_size(reg.reg[r].tag)) &&
                ...
                switch(tag) {
                case RISCV64_CPU_CACHE_TAG_L1_ONLY:
                    level1_only = 1;
                    break;
                case RISCV64_CPU_CACHE_TAG_LEAF4:
                    for (l4 = 0; cache_count < UCS_CPU_CACHE_LAST; l4++) {
                        ucs_x86_cpuid_ecx(RISCV64_CPUID_GET_LEAF4_INFO, l4,
