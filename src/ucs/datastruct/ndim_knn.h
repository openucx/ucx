...
#define __NDIM_WNN_PROTOTYPES(name, K, ndim, khkey_t, khval_t, coeff_t) \
	...
	extern wnn_t(name) *wnn_init_##name##_inplace(wnn_t(name) *wnn, \
												  coeff_t approximation, \
...
#define __NDIM_KNN_IMPL(name, K, ndim, SCOPE, khkey_t, khval_t, sum_t, \
	...
	SCOPE int knn_init_common_##name(knn_t(name) *knn) { \
		size_t i; \
		if (ndh_init_mpool(&knn->super.node_mp, #name, sizeof(nn_node_t(name)))) { \
			return -1; \
		} \
		\
		for (i = 0; i < ndim; i++) { \
	...
	{ \
		unsigned i; \
		nn_node_t(name)* node = (nn_node_t(name)*) \
								ucs_mpool_get_inline(&knn->super.node_mp); \
		for (i = 0; i < ndim; i++) { \
	...
	SCOPE int knn_insert_##name(knn_t(name) *knn, const khkey_t key[ndim], khval_t value) \
	{ \
	...
	SCOPE int knn_update_node_##name##_internal(knn_t(name) *knn, \
												const sum_t inc[ndim], \
												unsigned dim, \
									 			nn_node_t(name) *node, \
									 			nn_node_t(name) *nearest[K + 1]) \
	{ \
	...
	static int knn_update_node_##name(knn_t(name) *knn, unsigned dim, \
									  ...
									  nn_node_t(name) *nearest[K + 1]) \
	{ \
		return knn_update_node_##name##_internal(knn, NULL, dim, node, nearest); \
	} \
	...
	{ \
		...
		for (i = 0; i < ndim; i++) { \
			...
			while (ucs_likely(iter != kh_end(h))) { \
				...
				if ((iter = ((iter + ++step) & mask)) == first) break; \
...
#define __NDIM_ANN_IMPL(name, K, ndim, SCOPE, khkey_t, khval_t, coeff_t, \
	...
	SCOPE int ann_update_node_##name##_internal(knn_t(name) *knn, \
												const coeff_t inc[ndim], \
												unsigned dim, \
												nn_node_t(name) *node, \
												nn_node_t(name) *nearest[K+1]) \
	{ \
	...
	static int ann_update_node_##name(knn_t(name) *knn, unsigned dim, \
									  ...
									  nn_node_t(name) *nearest[K+1]) \
	{ \
		return ann_update_node_##name##_internal(knn, NULL, dim, node, nearest); \
	} \
...
#define __NDIM_WNN_IMPL(name, K, ndim, SCOPE, khkey_t, khval_t, coeff_t, \
	...
	__NDIM_ANN_IMPL(name, K, ndim, SCOPE, khkey_t, khval_t, coeff_t, \
					__khash_func, __khash_equal, __vhash_func, __vhash_equal) \
	SCOPE int wnn_init_common_##name(wnn_t(name) *wnn, \
									 coeff_t approximation, \
									 const coeff_t coefficients[ndim]) { \
		coeff_t next, sum = 0; \
		unsigned i, j, k, min; \
		...
		for (i = 0; i < ndim; i++) { \
			for (min = ndim, j = 0; j < ndim; j++) { \
				...
				if ((k == i) /* j isn't among last i enties of order[] */ && \
					((min == ndim) || (coefficients[j] < coefficients[min]))) { \
					min = j; \
				} \
			} \
			if (min == ndim) { \
	...
	SCOPE wnn_t(name) *wnn_init_##name(coeff_t approximation, \
									   const coeff_t coeff[ndim]) { \
		wnn_t(name) *wnn = (wnn_t(name)*)kcalloc(1, sizeof(wnn_t(name))); \
		if ((wnn != NULL) && (wnn_init_common_##name(wnn, approximation, \
													 coeff))) { \
			kfree(wnn); \
			wnn = NULL; \
		} \
	...
	SCOPE int wnn_insert_##name(wnn_t(name) *wnn, const khkey_t key[ndim], \
								khval_t value) \
	{ \
		return ann_insert_##name##_internal(&wnn->super, wnn->order, key, value); \
	} \
...
#define NDIM_ANN_INIT(name, K, ndim, khkey_t, khval_t, coeff_t, \
					  __khash_func, __khash_equal, \
					  __vhash_func, __vhash_equal) \
	NDIM_ANN_INIT2(name, K, ndim, static kh_inline klib_unused, khkey_t, \
				   khval_t, coeff_t, __khash_func, __khash_equal, __vhash_func, \
				   __vhash_equal)

#define NDIM_WNN_INIT(name, K, ndim, khkey_t, khval_t, coeff_t, \
					  __khash_func, __khash_equal, \
					  __vhash_func, __vhash_equal) \
...
/*! @function
  ...
  @param  c      Coefficients for the wighted distance function [array of numbers]
 */
#define wnn_init_inplace(prefix, name, h, a, c) wnn_init_##name##_inplace(h, a, c)
...
/*! @function
  ...
  @param  h      Pointer to the n-dimensional ANN lookup [nn_t(prefix, name)*]
 */
#define nn_destroy_inplace(prefix, name, h) prefix##nn_destroy_##name##_inplace(h)
