...
#define __NDIM_HASH_PROTOTYPES(name, ndim, khkey_t, khval_t) \
	...
	extern int ndh_insert_##name(ndhash_t(name) *h, const khkey_t key[ndim], khval_t value); \
	extern int ndh_anyv_##name(ndhash_t(name) *h, unsigned cnt, ndnode_t(name)** res); \
	extern int ndh_delv_##name(ndhash_t(name) *h, khval_t value);

#define __NDIM_HASH_IMPL(name, ndim, SCOPE, khkey_t, khval_t, __khash_func, __khash_equal, __vhash_func, __vhash_equal) \
	__KHASH_IMPL(name,           SCOPE, khkey_t, ndnode_t(name)*, 1, 1, __khash_func, __khash_equal) \
	__KHASH_IMPL(name##_reverse, SCOPE, khval_t, ndnode_t(name)*, 1, 1, __vhash_func, __vhash_equal) \
	SCOPE ndhash_t(name) *ndh_init_##name(void) { \
		ndhash_t(name) *ndh = (ndhash_t(name)*)kcalloc(1, sizeof(ndhash_t(name))); \
		if ((ndh != NULL) && (ndh_init_mpool(&ndh->node_mp, #name, sizeof(ndnode_t(name))))) { \
			kfree(ndh); \
			ndh = NULL; \
		} \
		return ndh; \
	...
	SCOPE ndhash_t(name) *ndh_init_##name##_inplace(ndhash_t(name) *h) { \
		return ((!ndh_init_mpool(&h->node_mp, #name, sizeof(ndnode_t(name))) && \
	...
	SCOPE int ndh_insert_##name##_node(ndhash_t(name) *h, ndnode_t(name)* node) \
	{ \
		int ret; \
		unsigned i; \
		khiter_t iter; \
		...
		if (ret == UCS_KH_PUT_FAILED) return -1; \
	...
	SCOPE int ndh_insert_##name(ndhash_t(name) *h, khkey_t key[ndim], khval_t value) \
	{ \
		ndnode_t(name)* node = (ndnode_t(name)*)ucs_mpool_get_inline(&h->node_mp); \
		if (node == NULL) { \
			return -1; \
		} \
		node->value = value; \
	...
	SCOPE int ndh_anyv_##name(ndhash_t(name) *h, unsigned cnt, ndnode_t(name)** res) \
	{ \
...
static ucs_mpool_ops_t ndh_mpool_ops = {
...
static inline int ndh_init_mpool(ucs_mpool_t *mp, const char *name, size_t elem_size)
{
...
#define NDIM_HASH_INIT2(name, ndim, SCOPE, khkey_t, khval_t, __khash_func, __khash_equal, __vhash_func, __vhash_equal) \
	__NDIM_HASH_TYPE(name, ndim, khkey_t, khval_t) \
	__NDIM_HASH_IMPL(name, ndim, SCOPE, khkey_t, khval_t, __khash_func, __khash_equal, __vhash_func, __vhash_equal) \

#define NDIM_HASH_INIT(name, ndim, khkey_t, khval_t, __khash_func, __khash_equal, __vhash_func, __vhash_equal) \
	NDIM_HASH_INIT2(name, ndim, static kh_inline klib_unused, khkey_t, khval_t, __khash_func, __khash_equal, __vhash_func, __vhash_equal) \

#define NDIM_HASH_TYPE(name, ndim, khkey_t, khval_t) \
	__NDIM_HASH_TYPE(name, ndim, khkey_t, khval_t)

#define NDIM_HASH_IMPL(name, ndim, khkey_t, khval_t, __khash_func, __khash_equal, __vhash_func, __vhash_equal) \
	__NDIM_HASH_IMPL(name, ndim, static ndh_inline klib_unused, khkey_t, khval_t, __khash_func, __khash_equal, __vhash_func, __vhash_equal) \
...
#define NDIM_HASH_INIT_INT(name, ndim) \
	NDIM_HASH_INIT(name, ndim, khint32_t, uintptr_t, \
...
#define NDIM_HASH_INIT_INT64(name, ndim) \
	NDIM_HASH_INIT(name, ndim, khint64_t, uintptr_t, \
				   kh_int_hash_func, kh_int_hash_equal, \
				   kh_int64_hash_func, kh_int64_hash_equal)
...
#define NDIM_HASH_INIT_STR(name, ndim) \
	NDIM_HASH_INIT(name, ndim, kh_cstr_t, uintptr_t, \
