...
struct ucs_callbackq_elem {
    ...
    void             (*cb)(ucs_callbackq_elem_t *self);
    uint32_t         sn;
} UCS_S_PACKED;
...
{
    ucs_assert(ucs_queue_is_empty(&cbq->queue) ||
               UCS_CIRCULAR_COMPARE32(elem->sn, >=,
...
/**
 * Remove and execute all callbacks with sequence number less-than the one provided.
 *
 * @param cbq    Callback queue to execute callback from.
 * @param sn     Provided sequence number.
 */
static inline unsigned ucs_callbackq_pull(ucs_callbackq_t *cbq, uint32_t sn)
{
    ...
    while (!ucs_queue_is_empty(&cbq->queue)) {
        elem = ucs_queue_head_elem_non_empty(&cbq->queue, ucs_callbackq_elem_t,
                                             queue);
        if (!UCS_CIRCULAR_COMPARE32(elem->sn, <, sn)) {
            break;
        }
