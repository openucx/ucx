...
#define ucs_log_dump_hex_buf(_data, _length) \
    ucs_log_dump_hex_buf_lvl(_data, _length, UCS_LOG_LEVEL_TRACE_DATA)
#define ucs_log_dump_hex_buf_lvl(_data, _length, _level) \
    do { \
        if (ucs_log_is_enabled(_level) || ucs_global_opts.log_print_queues) { \
            ucs_log_dump_hex_buf_impl(__FILE__, __LINE__, __FUNCTION__, \
                                      ucs_global_opts.log_print_queues ? \
                                      UCS_LOG_LEVEL_PRINT : _level, \
