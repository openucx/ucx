...
static ucs_config_field_t ucm_global_config_table[] = {
    {"LOG_LEVEL", "warn", "Logging level for memory events",
     ucs_offsetof(ucm_global_config_t, log_level),
     UCS_CONFIG_TYPE_ENUM(ucm_log_level_names)},

    {"ALLOC_ALIGN", "16", "Minimal alignment of allocated blocks",
     ucs_offsetof(ucm_global_config_t, alloc_alignment),
     UCS_CONFIG_TYPE_MEMUNITS},

    {"EVENTS", "yes", "Enable memory events",
    ...
    {"MALLOC_HOOKS", "yes", "Enable using glibc malloc hooks",
    ...
    {"MALLOC_RELOC", "yes",
     ...
     ucs_offsetof(ucm_global_config_t, enable_malloc_reloc),
     UCS_CONFIG_TYPE_BOOL},
    ...
    {"CUDA_HOOK_MODE",
     ...
     "          part of the application is linked with Cuda runtime "
     "statically,\n"
    ...
    {"ROCM_HOOK_MODE",
     ...
     " reloc  - Use ELF relocation table to set hooks. In this mode, if a "
     "caller\n"
     "          resolves the HSA memory APIs via dlopen/dlsym rather than the "
     "GOT,\n"
     "          its allocations may be missed and reported as host memory.\n"
 #if UCM_BISTRO_HOOKS
     "\n bistro - Use binary instrumentation to set hooks. In this mode, "
     "calls\n"
     "          into the HSA runtime are intercepted regardless of how the "
     "symbol\n"
    ...
    {"BISTRO_GRACE_DURATION", "50ms",
     ...
     "applying bistro patches. This allows threads currently executing "
     "patched\n"
