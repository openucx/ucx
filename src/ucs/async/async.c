...
static ucs_async_ops_t ucs_async_poll_ops = {
    ...
    .context_cleanup    = ucs_async_context_cleanup_empty,
    .context_try_block  = ucs_async_poll_tryblock,
    .context_unblock    = ucs_async_context_unblock_empty,
    .add_event_fd       = (ucs_async_add_event_fd_t)ucs_empty_function_return_success,
    .remove_event_fd    = ucs_async_remove_event_fd_empty,
    .modify_event_fd    = (ucs_async_modify_event_fd_t)ucs_empty_function_return_success,
    .add_timer          = ucs_async_add_timer_empty,
    .remove_timer       = ucs_async_remove_timer_empty,
};
...
void ucs_async_global_cleanup()
{
    ...
    if (num_elems != 0) {
        ucs_diag("async handler table is not empty during exit (contains %d "
                 "elems)",
                 num_elems);
    }
