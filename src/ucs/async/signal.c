...
#define UCS_SIGNAL_TIMER_IDX_SHIFT (6)
#define UCS_SIGNAL_MAX_TIMERQS     (UCS_BIT(6))
                                  ...
                                  ucs_time_t interval, unsigned *timer_id_p)
{
    ...
    *timer_id_p = (*timer_id_p << UCS_SIGNAL_TIMER_IDX_SHIFT) |
                  (uid & UCS_MASK(UCS_SIGNAL_TIMER_IDX_SHIFT));
                                     ...
                                     unsigned timer_id)
{
    ...
    ucs_assert((timer_id & UCS_MASK(UCS_SIGNAL_TIMER_IDX_SHIFT)) ==
               (timer - ucs_async_signal_global_context.timers));

    timer_id = timer_id >> UCS_SIGNAL_TIMER_IDX_SHIFT;
