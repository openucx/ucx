...
extern int
filter_node_sum(ucs_stats_filter_node_t *node);
extern int
filter_all_node_sum(ucs_stats_filter_node_t *node);
extern ucs_status_t
...
if (filter_node_sum(&ucs_stats_context.root_filter_node) > 2000) {
printf("node sum %d\n", node_sum(&ucs_stats_context.root_node));
printf("filter node sum %d\n", filter_node_sum(&ucs_stats_context.root_filter_node));
printf("filter ALLL node sum %d\n", filter_all_node_sum(&ucs_stats_context.root_filter_node));
printf("filter_parent->child_len %d\n", filter_parent->child_len);
//    ucs_stats_serialize_text_recurs_filtered(stdout, &ucs_stats_context.root_filter_node, 3);
//    abort();
}
    ucs_stats_node_t * t_node3 = ucs_list_head(&filter_parent->type_list_head,
                            ucs_stats_node_t,
                             type_list);

              printf("filter_child_temp->name %s %s \n", t_node3->cls->name, t_node3->name);
