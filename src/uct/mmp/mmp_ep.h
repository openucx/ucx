...
typedef struct uct_mmp_ep_addr {
    uct_ep_addr_t     super;
    int               ep_id;
} uct_mmp_ep_addr_t;

typedef struct uct_mmp_ep {
    uct_ep_t          super;
    gni_ep_handle_t   ep;
    unsigned          outstanding;
    uintptr_t         hash_key;
    struct uct_mmp_ep *next;
} uct_mmp_ep_t;

UCS_CLASS_DECLARE_NEW_FUNC(uct_mmp_ep_t, uct_ep_t, uct_iface_t*);
UCS_CLASS_DECLARE_DELETE_FUNC(uct_mmp_ep_t, uct_ep_t);
...
ucs_status_t uct_mmp_ep_put_short(uct_ep_h tl_ep, void *buffer, unsigned length, 
