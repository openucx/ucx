...
static ucs_status_t uct_posix_mmap(void **address_p, size_t *length_p, int flags,
...
static uint64_t uct_posix_segment_open(uct_mm_md_t *md,
                                          uct_mm_seg_id_t *seg_id_p, int *fd_p)
{
    uct_posix_md_config_t *posix_config = ucs_derived_of(md->config,
                                                         uct_posix_md_config_t);
    uint64_t mmid, flags;
                    ...
                    unsigned flags, const char *alloc_name, uct_mem_h *memh_p)
{
    ...
    if (posix_config->use_proc_link) {
        ...
        seg->seg_id = uct_posix_mmid_procfs_pack(fd) | UCT_POSIX_SEG_FLAG_PROCFS;
    }
