                                  ...
                                  const void *payload, unsigned length)
{
    ...
    uct_am_short_fill_data(&packet->data, header, payload, length);
    //memcpy(ep->buf + sizeof(sisci_packet_t), payload, length);
    SCIFlush(NULL, SCI_NO_FLAGS);    
    packet->status = 1;
    SCIFlush(NULL, SCI_NO_FLAGS);

    printf("what we are sending: %s\n", (char*) ep->buf + sizeof(sisci_packet_t));
...
static void uct_sci_process_recv(uct_iface_h tl_iface) {
    ...
    ucs_status_t status;
    status = uct_iface_invoke_am(&iface->super, packet->am_id, &packet->data, packet->length,0);
