...
typedef struct uct_iface_mpool_config {
    ...
    unsigned          bufs_start; /* How many buffers (approx.) are allocated on first time */
 ...
 */
#define UCT_IFACE_MPOOL_CONFIG_FIELDS(_prefix, _dfl_max, _dfl_start, _dfl_grow, _mp_name, _offset, _desc) \
    ...
    \
    {_prefix "BUFS_START", UCS_PP_QUOTE(_dfl_start), \
     "How much buffers are added on " _mp_name " memory pool starts\n" \
     "0 means the value is chosen by the transport.", \
     (_offset) + ucs_offsetof(uct_iface_mpool_config_t, bufs_start), UCS_CONFIG_TYPE_UINT}, \
    \
 ...
 * @param start        Default number of buffers added for start chunk.
