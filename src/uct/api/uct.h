 ...
 * the specified @a stream. This call returns an @a op_event, which is used to
 ...
 * @param [out] op_event  Operation event used for subsequent tracking.
 *
 * @return Error code.
 */
...
{
    return iface->ops.iface_stream_op_begin(iface, stream, op_event);
}
 ...
 * creation of the operation represented by @a op_event has completed.
 ...
 *
 * @param [in]  iface     Interface used for state management.
 * @param [in]  op_event  Operation event previously returned by
...
{
    return iface->ops.iface_stream_op_is_ready(iface, op_event);
}
 ...
 *
 * Declares that the operation associated with @a op_event has completed
 * its processing. This will release the wait on the stream associated with
 * @a op_event, unblock any subsequent operations that were waiting on
 * this stream, and release the resources associated with @a op_event.
 *
 * @param [in]  iface     Interface used for state management.
 * @param [in]  op_event  Operation event previously returned by
 *                        @ref uct_iface_stream_op_begin.
...
UCT_INLINE_API ucs_status_t uct_iface_stream_op_finish(
        const uct_iface_h iface, uct_iface_stream_op_event_h op_event)
{
    return iface->ops.iface_stream_op_finish(iface, op_event);
}
