...
typedef struct {
    ...
    /**
     * Memory flags associated with local memory type.
     * This field must be initialized by the caller.
     */
    uint8_t             local_memory_flags;

    /**
     * Memory flags associated with remote memory type.
     * This field must be initialized by the caller.
     */
    uint8_t             remote_memory_flags;

    /**
     * Distance to the peer which can be obtained using @ref
     * uct_iface_is_reachable_v2.
     * This field must be initialized by the caller.
     */
    uint16_t            distance;
...
typedef struct uct_iface_is_reachable_params {
    ...
    /**
     * An opaque UCT object representing distance to the peer. It can further be
     * used in @uct_iface_estimate_perf. This field is set by the UCT layer.
     */
    uint16_t                      distance;
