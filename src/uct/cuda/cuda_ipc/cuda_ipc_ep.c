...
static UCS_CLASS_INIT_FUNC(uct_cuda_ipc_ep_t, const uct_ep_params_t *params)
{
    ...
    iface_addr_length       = UCT_EP_PARAM_VALUE(params, iface_addr_length,
                                                 IFACE_ADDR_LENGTH, 0);
    self->remote_pid_ns.pid = uct_cuda_ipc_iface_address_pid(iface_addr);
    self->remote_pid_ns.ns  = uct_cuda_ipc_iface_address_pid_ns(
            iface_addr, iface_addr_length);
    self->device_ep         = NULL;
    return UCS_OK;
}
...
{
    uct_cuda_ipc_destroy_cache_by_pid_ns(self->remote_pid_ns);
                                 ...
                                 const uct_ep_is_connected_params_t *params)
{
    ...
    return ep->remote_pid_ns.pid ==
           uct_cuda_ipc_iface_address_pid(params->iface_addr);
}
