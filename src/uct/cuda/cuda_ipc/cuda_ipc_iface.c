...
static unsigned
uct_cuda_ipc_progress_event_queue(uct_iface_h tl_iface,
                                  ...
                                  unsigned max_events)
{
    ...
    ucs_queue_for_each_safe(cuda_ipc_event, iter, event_q, super.queue) {
        status = UCT_CUDADRV_FUNC_LOG_ERR(cuEventQuery(cuda_ipc_event->super.event));
        ...
        if (cuda_ipc_event->super.comp != NULL) {
            uct_invoke_completion(cuda_ipc_event->super.comp, UCS_OK);
        }
...
{
    uct_cuda_ipc_iface_t *iface     = ucs_derived_of(tl_iface, uct_cuda_ipc_iface_t);
    uct_cuda_ipc_ctx_rsc_t *ctx_rsc = ucs_derived_of(base, uct_cuda_ipc_ctx_rsc_t);
    unsigned i;

    for (i = 0; i < iface->config.max_streams; i++) {
        uct_cuda_base_queue_desc_destroy(base, &ctx_rsc->queue_desc[i]);
    }
}
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    self->super.ops.create_rsc     = uct_cuda_ipc_ctx_rsc_create;
    self->super.ops.destroy_rsc    = uct_cuda_ipc_ctx_rsc_destroy;
    self->super.ops.progress_queue = uct_cuda_ipc_progress_event_queue;
    self->super.max_events         = self->config.max_cuda_ipc_events;
    self->super.max_poll           = self->config.max_poll;
    self->super.event_size         = sizeof(uct_cuda_ipc_event_desc_t);
    return UCS_OK;
}
