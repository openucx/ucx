...
static ucs_status_t uct_cuda_ipc_vmm_multi_check_layout(
        const uct_cuda_ipc_vmm_multi_info_t *info,
        ucs_log_level_t log_level)
{
    size_t chunks_size;

    /* The inline information and chunk descriptors are part of the protocol. */
    UCS_STATIC_ASSERT(sizeof(uct_cuda_ipc_vmm_multi_info_t) == 16);
    UCS_STATIC_ASSERT(sizeof(uct_cuda_ipc_vmm_multi_info_t) <=
                      sizeof(CUmemPoolPtrExportData));
    UCS_STATIC_ASSERT(sizeof(uct_cuda_ipc_vmm_chunk_desc_t) == 88);

    if (info->version != UCT_CUDA_IPC_VMM_MULTI_VERSION) {
        ucs_log(log_level,
                "unsupported multi-chunk VMM metadata version %u, expected %u",
                info->version, UCT_CUDA_IPC_VMM_MULTI_VERSION);
        return UCS_ERR_UNSUPPORTED;
    }

    if ((info->info_size < sizeof(*info)) ||
        (info->chunk_desc_size != sizeof(uct_cuda_ipc_vmm_chunk_desc_t))) {
        ucs_log(log_level,
                "unsupported multi-chunk VMM metadata layout: info %u chunk "
                "descriptor %u, expected at least %zu and exactly %zu",
                info->info_size, info->chunk_desc_size, sizeof(*info),
                sizeof(uct_cuda_ipc_vmm_chunk_desc_t));
        return UCS_ERR_UNSUPPORTED;
    }

    if (info->num_chunks == 0) {
        ucs_log(log_level, "multi-chunk VMM metadata has no chunks");
        return UCS_ERR_INVALID_PARAM;
    }

    chunks_size = info->num_chunks * sizeof(uct_cuda_ipc_vmm_chunk_desc_t);
    if (info->alloc_size < chunks_size) {
        ucs_log(log_level,
                "multi-chunk VMM metadata allocation size %zu is smaller than "
                "descriptor data size %zu",
                info->alloc_size, chunks_size);
        return UCS_ERR_INVALID_PARAM;
    }
...
void uct_cuda_ipc_vmm_multi_meta_cleanup(uct_cuda_ipc_lkey_t *key)
{
    ...
    ucs_list_for_each_safe(meta, tmp, &key->vmm_multi_list, list) {
        UCT_CUDADRV_FUNC_LOG_WARN(cuMemUnmap(meta->dev_ptr, meta->alloc_size));
        UCT_CUDADRV_FUNC_LOG_WARN(
                cuMemAddressFree(meta->dev_ptr, meta->alloc_size));

        ucs_list_del(&meta->list);
        ucs_free(meta);
    }
}
                                       ...
                                       uint16_t *num_chunks_p)
{
    ...
    for (pos = va_base; pos < va_base + va_len; pos = chunk_base + chunk_size) {
        ...
        status = UCT_CUDADRV_FUNC_LOG_ERR(
                cuPointerGetAttribute(&chunk_buffer_id,
        ...
        elem = ucs_array_append(&chunks, {
            ...
            ucs_error("failed to allocate VMM chunk descriptor");
                                          ...
                                          int dev_num)
{
    ...
    status = uct_cuda_ipc_vmm_multi_meta_alloc_buffer(
            ...
            &prop, alloc_granularity, dev_num);
    if (status != UCS_OK) {
        goto err_free_host;
    }
        ...
        size_t length, const uct_cuda_ipc_vmm_multi_meta_t **meta_p)
{
    ...
    ucs_list_for_each(meta, &key->vmm_multi_list, list) {
    ...
    status = uct_cuda_ipc_vmm_multi_create_meta_buffer(meta, memh->dev_num);
    if (status != UCS_OK) {
        goto err_free_meta;
    }

    /* Peers may unpack any published metadata until memh deregistration. */
    ucs_list_add_tail(&key->vmm_multi_list, &meta->list);
    *meta_p = meta;
    goto out_pop;
...
{
    const uct_cuda_ipc_vmm_multi_info_t *info =
            &rkey->super.super.ph.vmm_multi;
    CUmemGenericAllocationHandle handle;
    CUdeviceptr dev_ptr;
    size_t chunks_size;
    ucs_status_t status;

    status = uct_cuda_ipc_vmm_multi_check_layout(info, log_level);
    if (status != UCS_OK) {
        return status;
    }

    chunks_size = info->num_chunks * sizeof(*rkey->chunks);

    rkey->chunks = ucs_malloc(chunks_size, "vmm_multi_chunks");
    if (rkey->chunks == NULL) {
        ucs_error("failed to allocate %zu bytes for VMM chunk descriptors",
                  chunks_size);
        return UCS_ERR_NO_MEMORY;
    }
    rkey->num_chunks = info->num_chunks;

    status = uct_cuda_ipc_vmm_multi_fetch_meta_import(
            &rkey->super.super.ph.handle.fabric_handle, cu_dev,
            info->alloc_size, &dev_ptr, &handle, log_level);
    if (status != UCS_OK) {
        goto err_free;
    }

    status = UCT_CUDADRV_FUNC(cuMemcpyDtoH(rkey->chunks, dev_ptr, chunks_size),
                              log_level);
    uct_cuda_ipc_vmm_multi_fetch_meta_release(dev_ptr, info->alloc_size,
                                              handle);
    if (status != UCS_OK) {
        goto err_free;
    }
