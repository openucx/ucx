...
static ucs_status_t
uct_cuda_ipc_vmm_multi_check_handle(const uct_cuda_ipc_vmm_handle_t *vmm_handle,
                                    ucs_log_level_t log_level)
{
    if (vmm_handle->handle_type != UCT_CUDA_IPC_KEY_HANDLE_TYPE_VMM) {
        ucs_log(log_level, "unsupported multi-chunk VMM handle type %d",
                vmm_handle->handle_type);
...
static ucs_status_t uct_cuda_ipc_vmm_multi_check_layout(
        const uct_cuda_ipc_vmm_meta_header_t *header,
        ucs_log_level_t log_level)
{
    if ((header->header_size < sizeof(*header)) ||
        (header->chunk_desc_size != sizeof(uct_cuda_ipc_vmm_chunk_desc_t))) {
        ucs_log(log_level,
                "unsupported multi-chunk VMM metadata layout: header %u chunk "
                "descriptor %u, expected at least %zu and exactly %zu",
                header->header_size, header->chunk_desc_size, sizeof(*header),
                                       ...
                                       uint64_t *buffer_id_p)
{
    ...
    for (pos = va_base; pos < va_base + va_len; pos = chunk_base + chunk_size) {
        ...
        /* The buffer ID is the sum of all chunk buffer IDs in the region */
        buffer_id += chunk_buffer_id;
                                    ...
                                    CUdevice cu_dev, ucs_log_level_t log_level)
{
    ...
    /* The header and the chunk descriptors reach the peer as raw bytes through
     * GPU memory, so their layout is part of the protocol. */
    UCS_STATIC_ASSERT(sizeof(uct_cuda_ipc_vmm_handle_t) == 72);
    UCS_STATIC_ASSERT(sizeof(uct_cuda_ipc_vmm_chunk_desc_t) == 88);
    UCS_STATIC_ASSERT(sizeof(uct_cuda_ipc_vmm_meta_header_t) == 80);
    ...
    status = uct_cuda_ipc_vmm_multi_check_layout(&header, log_level);
    if (status != UCS_OK) {
        return status;
    }

    status = uct_cuda_ipc_vmm_multi_check_handle(&header.chunks_handle,
                                                 log_level);
    if (status != UCS_OK) {
        return status;
    }
