                                                    ...
                                                    unsigned events)
{
    ...
    if (uct_cuda_copy_progress_event_queue(iface, UCT_CUDA_COPY_STREAM_H2D,
                                           0, iface->config.max_poll)) {
        return UCS_ERR_BUSY;
    }

    if (uct_cuda_copy_progress_event_queue(iface, UCT_CUDA_COPY_STREAM_D2H,
                                           0, iface->config.max_poll)) {
        return UCS_ERR_BUSY;
    }
