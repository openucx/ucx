...
static int uct_cuda_copy_md_is_dmabuf_supported()
{
    ...
    if (UCT_CUDA_FUNC_LOG_DEBUG(cuDeviceGet, &cuda_device, 0) != UCS_OK) {
        return 0;
    }

    /* Assume dmabuf support is uniform across all devices */
#if CUDA_VERSION >= 11070
    if (UCT_CUDA_FUNC_LOG_DEBUG(cuDeviceGetAttribute, &dmabuf_supported,
                 ...
                 const uct_md_mem_reg_params_t *params, uct_mem_h *memh_p)
{
    ...
    log_level = (flags & UCT_MD_MEM_FLAG_HIDE_ERRORS) ? UCS_LOG_LEVEL_DEBUG :
                UCS_LOG_LEVEL_ERROR;
    status    = UCT_CUDA_FUNC(log_level, cuMemHostRegister, address, length,
                 ...
                 uct_md_h md, const uct_md_mem_dereg_params_t *params)
{
    ...
    if (params->memh != &uct_cuda_dummy_memh) {
        UCT_CUDA_FUNC_LOG_DIAG(cuMemHostUnregister, (void*)params->memh);
    }
                               ...
                               CUdevice cu_device, unsigned flags)
{
    ...
    if (md->granularity == SIZE_MAX) {
        status = UCT_CUDA_FUNC(log_level, cuMemGetAllocationGranularity,
                               &md->granularity, &prop,
    ...
    status = UCT_CUDA_FUNC(log_level, cuMemCreate,
                           &alloc_handle->generic_handle, alloc_handle->length,
                           &prop, 0);
    if (status != UCS_OK) {
        return UCS_ERR_NO_MEMORY;
    }

    status = UCT_CUDA_FUNC(log_level, cuMemAddressReserve, &alloc_handle->ptr,
                           alloc_handle->length, md->granularity, 0, 0);
    if (status != UCS_OK) {
        goto err_mem_release;
    }

    status = UCT_CUDA_FUNC(log_level, cuMemMap, alloc_handle->ptr,
    ...
    status = UCT_CUDA_FUNC(log_level, cuMemSetAccess, alloc_handle->ptr,
                           alloc_handle->length, &access_desc, 1);
    if (status != UCS_OK) {
        goto err_mem_unmap;
    }

    status = UCT_CUDA_FUNC(log_level, cuPointerGetAttribute, &allowed_types,
...
err_mem_unmap:
    UCT_CUDA_FUNC_LOG_DEBUG(cuMemUnmap, alloc_handle->ptr,
                            alloc_handle->length);
err_address_free:
    UCT_CUDA_FUNC_LOG_DEBUG(cuMemAddressFree, alloc_handle->ptr,
                            alloc_handle->length);
err_mem_release:
    UCT_CUDA_FUNC_LOG_DEBUG(cuMemRelease, alloc_handle->generic_handle);
#endif
    return UCS_ERR_NO_MEMORY;
}
...
static ucs_status_t uct_cuda_copy_set_ctx_sync_memops(int log_level)
{
    ...
    if (cuda_cuCtxSetFlags_func != NULL) {
        ...
        UCT_CUDA_FUNC(log_level, cuda_cuCtxSetFlags_func, CU_CTX_SYNC_MEMOPS);
        return UCS_OK;
    }
#endif
...
static void uct_cuda_copy_sync_memops(CUdeviceptr dptr, int is_vmm)
{
    ...
    UCT_CUDA_FUNC_LOG_WARN(cuPointerSetAttribute, &sync_memops_value,
                                    ...
                                    ucs_log_level_t log_level)
{
    ...
    status = UCT_CUDA_FUNC(log_level, cuCtxPushCurrent, primary_ctx);
    if (status != UCS_OK) {
        (void)UCT_CUDA_FUNC(log_level, cuDevicePrimaryCtxRelease, device);
    }
                             ...
                             ucs_log_level_t log_level)
{
    ...
    status = UCT_CUDA_FUNC_LOG_DEBUG(cuCtxGetDevice, cu_device_p);
    if (status != UCS_OK) {
    ...
    status = UCT_CUDA_FUNC_LOG_DIAG(cuDeviceGetCount, &num_devices);
    if (status != UCS_OK) {
        return UCS_ERR_INVALID_PARAM;
    }
    ...
    for (dev_ordinal = 0; dev_ordinal < num_devices; dev_ordinal++) {
        if (UCT_CUDA_FUNC_LOG_DEBUG(cuDeviceGet, alloc_cu_device_p,
                                    dev_ordinal) != UCS_OK) {
            continue;
        }
...
{
    (void)UCT_CUDA_FUNC_LOG_WARN(cuCtxPopCurrent, NULL);
    (void)UCT_CUDA_FUNC_LOG_WARN(cuDevicePrimaryCtxRelease, cu_device);
}
                        ...
                        uct_mem_h *memh_p)
{
    ...
    if (mem_type == UCS_MEMORY_TYPE_CUDA) {
        ...
        if (md->config.enable_fabric != UCS_YES) {
            status = UCT_CUDA_FUNC(log_level, cuMemAlloc, &alloc_handle->ptr,
                                   alloc_handle->length);
            if (status == UCS_OK) {
                goto allocated;
            }
        ...
        status = UCT_CUDA_FUNC(log_level, cuMemAllocManaged, &alloc_handle->ptr,
                               alloc_handle->length, CU_MEM_ATTACH_GLOBAL);
    } else {
        ucs_log(log_level,
...
uct_cuda_copy_mem_release_fabric(uct_cuda_copy_alloc_handle_t *alloc_handle)
{
    ...
    status = UCT_CUDA_FUNC_LOG_WARN(cuMemRelease, alloc_handle->generic_handle);
    if (status != UCS_OK) {
        return status;
    }

    status = UCT_CUDA_FUNC_LOG_WARN(cuMemUnmap, alloc_handle->ptr,
                                    alloc_handle->length);
    if (status != UCS_OK) {
        return status;
    }

    return UCT_CUDA_FUNC_LOG_WARN(cuMemAddressFree, alloc_handle->ptr,
                                  alloc_handle->length);
#else
    return UCS_ERR_UNSUPPORTED;
#endif
                                    ...
                                    CUdevice *cuda_device)
{
    ...
    *cuda_device  = CU_DEVICE_INVALID;
    ...
    status = UCT_CUDA_FUNC_LOG_DEBUG(cuMemGetAllocationPropertiesFromHandle,
                                     &prop, alloc_handle);
    if (status != UCS_OK) {
        goto out;
    }
...
out:
    UCT_CUDA_FUNC_LOG_WARN(cuMemRelease, alloc_handle);
    return 1;
#else
    return 0;
#endif
...
static ucs_status_t uct_cuda_copy_mem_free(uct_md_h md, uct_mem_h memh)
{
    ...
    if (alloc_handle->is_vmm) {
        ...
        UCT_CUDA_FUNC_LOG_DIAG(cuMemFree, alloc_handle->ptr);
        status = UCS_OK;
    }
...
static size_t uct_cuda_copy_md_get_total_device_mem(CUdevice cuda_device)
{
    ...
    if (!total_bytes[cuda_device]) {
        if (UCT_CUDA_FUNC_LOG_ERR(cuDeviceTotalMem, &total_bytes[cuda_device],
                                  cuda_device) != UCS_OK) {
            goto err;
        }

        if (UCT_CUDA_FUNC_LOG_ERR(cuDeviceGetName, dev_name, sizeof(dev_name),
                                  cuda_device) != UCS_OK) {
            goto err;
        }
        ...
        ucs_memory_info_t *mem_info)
{
    ...
    if (UCT_CUDA_FUNC_LOG_DEBUG(cuMemGetAddressRange, &base_address,
...
out_ctx_pop:
    UCT_CUDA_FUNC_LOG_WARN(cuCtxPopCurrent, &tmp_ctx);
    if (cuda_ctx == NULL) {
...
#define UCT_CUDA_MEM_QUERY_NUM_ATTRS 4
    ...
    if (is_vmm) {
        ...
        status = UCT_CUDA_FUNC_LOG_ERR(cuPointerGetAttributes,
