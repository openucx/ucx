...
{
    uct_cuda_iface_t *iface = ucs_derived_of(tl_iface, uct_cuda_iface_t);
    unsigned max_events     = iface->max_poll;
    unsigned count          = 0;
    ucs_queue_head_t *event_q;
    uct_cuda_queue_desc_t *q_desc;
    ucs_queue_iter_t iter;
                                          ...
                                          uct_cuda_ctx_rsc_t **ctx_rsc_p)
{
    ...
    mp_params.elem_size       = iface->event_size;
...
static void uct_cuda_base_ctx_rsc_destroy(uct_cuda_ctx_rsc_t *ctx_rsc)
{
...
static UCS_CLASS_CLEANUP_FUNC(uct_cuda_iface_t)
{
    ...
    kh_foreach_value(&self->ctx_rscs, ctx_rsc, {
        self->ops.destroy_rsc(&self->super.super, ctx_rsc);
        uct_cuda_base_ctx_rsc_destroy(ctx_rsc);
    });
