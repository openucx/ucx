...
static ucs_status_t uct_gaudi_gdr_iface_query(uct_iface_h tl_iface,
                                              uct_iface_attr_t *iface_attr)
{
    uct_gaudi_gdr_iface_t *iface = ucs_derived_of(tl_iface, uct_gaudi_gdr_iface_t);

    uct_base_iface_query(&iface->super, iface_attr);
    iface_attr->cap.flags               = UCT_IFACE_FLAG_CONNECT_TO_IFACE;
    iface_attr->bandwidth.dedicated     = 0.0001; // DBL_MIN will be round down to 0 when packing address;
...
UCS_CLASS_DEFINE_NEW_FUNC(uct_gaudi_gdr_iface_t, uct_iface_t, uct_md_h, uct_worker_h,
                          const uct_iface_params_t*, const uct_iface_config_t*);

UCT_TL_DEFINE(&uct_gaudi_gdr_component, gaudi_gdr,
              uct_gaudi_base_query_devices, uct_gaudi_gdr_iface_t,
              "GAUDI_GDR_", uct_iface_config_table,
              uct_iface_config_t);
