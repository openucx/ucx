...
ze_result_t uct_ze_base_init(void)
{
    ...
    UCS_INIT_ONCE(&uct_ze_base.init_once) {
        ...
        for (device_idx = 0; device_idx < uct_ze_base.num_devices;
            ...
            /* Get standard device properties (includes PCI info in v1.3+) */
            ...
            ret = zeDeviceGetProperties(device->root_device,
                                        &device->device_props);
            if (ret != ZE_RESULT_SUCCESS) {
                ucs_debug("failure to get ze device properties for device %d: "
                          "0x%x",
                          device_idx, ret);
                continue;
            }
            ...
            /* Register with topology subsystem - ONCE per physical device */
            ...
            if (status != UCS_OK) {
                ...
                status = ucs_topo_sys_device_set_user_value(device->sys_dev,
                                                            device_idx);
                if (status == UCS_OK) {
                    /* Enable auxiliary path for multi-path routing (like CUDA) */
            ...
            ret = zeDeviceGetSubDevices(device->root_device, &subdev_count,
                                        NULL);
            if (ret != ZE_RESULT_SUCCESS) {
                ucs_debug("failure to get ze sub-devices count for device %d: "
                          "0x%x",
                          device_idx, ret);
                subdev_count = 0;
            }
            ...
            if (subdev_count == 0) {
                ...
                if (ret != ZE_RESULT_SUCCESS) {
                    ucs_debug(
                            "failure to get ze sub-devices for device %d: 0x%x",
                            device_idx, ret);
                    /* Fallback to root device as single sub-device */
                    device->num_subdevices = 1;
                    device->subdevices[0]  = device->root_device;
                } else {
            ...
            for (t = 0; t < device->num_subdevices; t++) {
                ...
                subdevice->device =
                        device; /* const pointer - device won't be modified */
                subdevice->subdevice_idx = t;
