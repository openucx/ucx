                       ...
                       unsigned max_log_sge)
{
    ...
    wr->wr.ud.remote_qpn = ep->dest_address.dest_qpn;
    wr->wr.ud.ah         = ep->dest_address.ah;
...
static void *
uct_ud_verbs_iface_get_peer_address(uct_ud_iface_t *iface,
                                    const uct_ib_address_t *ib_addr,
                                    const uct_ud_iface_addr_t *if_addr,
                                    int path_index, void *address_p)
{
    uct_ib_iface_t *ib_iface                = &iface->super;
    uct_ud_verbs_ep_address_t *peer_address = (uct_ud_verbs_ep_address_t*)address_p;
...
uct_ud_verbs_ep_create(const uct_ep_params_t *params, uct_ep_h *ep_p)
{
    if (ucs_test_all_flags(params->field_mask, UCT_EP_PARAM_FIELD_DEV_ADDR |
        ...
        return uct_ud_ep_create_connected(params->iface, params->dev_addr,
                                          ...
                                          ucs_offsetof(uct_ud_verbs_ep_t,
                                                       dest_address), ep_p);
    }
...
static const void *
uct_ud_verbs_iface_conn_match_get_address(const ucs_conn_match_elem_t *elem)
{
...
static const char*
uct_ud_verbs_iface_conn_match_address_str(const ucs_conn_match_ctx_t *conn_match_ctx,
                                          const void *address, char *str, size_t max_size)
{
    const uct_ud_verbs_ep_address_t *dest_address =
        (const uct_ud_verbs_ep_address_t*)address;
    ucs_snprintf_zero(str, max_size, "ah=%p dest_qpn=%u",
                      dest_address->ah, dest_address->dest_qpn);
    return str;
}

static const ucs_conn_match_ops_t uct_ud_verbs_iface_conn_match_ops = {
    .get_address = uct_ud_verbs_iface_conn_match_get_address,
    .get_conn_sn = uct_ud_iface_conn_match_get_conn_sn,
    .address_str = uct_ud_verbs_iface_conn_match_address_str
};
...
static uct_ud_iface_ops_t uct_ud_verbs_iface_ops = {
    ...
    .get_peer_address         = uct_ud_verbs_iface_get_peer_address
};
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    uct_ud_iface_cep_init(&self->super, sizeof(uct_ud_verbs_ep_address_t),
                          &uct_ud_verbs_iface_conn_match_ops);
