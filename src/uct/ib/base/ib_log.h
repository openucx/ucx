...
#define uct_ib_log_recv_completion(_iface, _qp_type, _l_qp, _r_qp, _slid, _data, _length, _dump_cb, ...) \
    if (ucs_log_enabled(UCS_LOG_LEVEL_TRACE_DATA)) { \
        __uct_ib_log_recv_completion(__FILE__, __LINE__, __FUNCTION__, \
                                     _iface, _qp_type, _l_qp, _r_qp, _slid, _data, _length, _dump_cb, ## __VA_ARGS__); \
    }
