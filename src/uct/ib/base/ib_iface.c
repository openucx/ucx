                                              ...
                                              const uct_ib_address_t *ib_addr)
{
    ...
    if (!is_local_eth && !(ib_addr->flags & UCT_IB_ADDRESS_FLAG_LINK_LAYER_ETH)) {
        flid = uct_ib_iface_flid_from_gid(iface, &params.gid);
        if (flid != 0) {
            /* Remote GID is site-local GID with non-zero FLID, therefore it
             * can be reachable from other subnets in the same collection. */
            return 1;
        }
...
static uint16_t uct_ib_gid_subnet_prefix(union ibv_gid *gid)
{
...
{
    union ibv_gid *lgid = &iface->gid_info.gid;

    if (!iface->config.flid_enabled) {
        /* FLID based routing disabled */
        return 0;
    }

    if ((gid->global.subnet_prefix & UCT_IB_SITE_LOCAL_FLID_MASK) !=
        UCT_IB_SITE_LOCAL_PREFIX) {
        /* GID format is not site-local (therefore does not contain FLID) */
        return 0;
    }

    if (uct_ib_gid_subnet_prefix(gid) == uct_ib_gid_subnet_prefix(lgid)) {
        /* GID subnet prefix matches ours - no need to use FLID */
        return 0;
    }

    return *((uint16_t*)UCS_PTR_BYTE_OFFSET(gid->raw, 4));
}
                                         ...
                                         enum ibv_mtu *path_mtu)
{
    ...
    if (ucs_test_all_flags(params.flags,
        ...
        params.flags & UCT_IB_ADDRESS_PACK_FLAG_ETH) {
        gid  = &params.gid;
        flid = uct_ib_iface_flid_from_gid(iface, gid);
    }

    lid = (flid == 0) ? params.lid : ntohs(flid);
