                                      ...
                                      int full_handshake)
{
    ...
    if (md->flags & UCT_IB_MLX5_MD_FLAG_DEVX_DCI) {
        ...
        attr->uidx                             =
            htonl(dci_index & 0xff) >> UCT_IB_UIDX_SHIFT;
                             ...
                             const uct_dc_mlx5_iface_config_t *config)
{
    ...
    ucs_debug("mlx5 verbs dct under rtr with ECE : 0x%x", dct->local_ece);
#endif
                             ...
                             const uct_dc_mlx5_iface_config_t *config)
{
    ...
    init_attr.create_flags    |= uct_dc_mlx5_iface_ooo_flag(iface,
                                                            ...
                                                            "DCT", 0);
    dct->verbs.dct = ibv_exp_create_dct(uct_ib_iface_device(&iface->super.super.super)->ibv_context,
                                        &init_attr);
                             ...
                             const uct_dc_mlx5_iface_config_t *config)
{
    ...
    iface->rx.dct[0].qp_num     = 0;
    iface->rx.dct[0].local_ece  = 0;
    iface->rx.dct[0].remote_ece = 0;
    iface->rx.dct[1].qp_num     = 0;
    iface->rx.dct[0].local_ece  = iface->super.super.super.ece;
    iface->rx.dct[1].remote_ece = iface->super.super.super.ece;
                                  ...
                                  const uct_dc_mlx5_iface_config_t *config)
{
    ...
    dct.local_ece  = UCT_IB_MLX5_DEVX_ECE_TRIG_RESP;
    dct.remote_ece = UCT_IB_MLX5_DEVX_ECE_TRIG_RESP;
    status = uct_dc_mlx5_iface_create_dct(iface, &dct, config);
    if (status != UCS_OK) {
        return status;
    }

    ib_iface->ece = dct.local_ece;
...
uct_dc_mlx5_iface_get_address(uct_iface_h tl_iface, uct_iface_addr_t *iface_addr)
{
    ...
    if (iface->rx.dct[1].qp_num) {
        addr->flags   |= UCT_DC_MLX5_IFACE_ADDR_DC_DCTN_ECE;
    }
...
static inline ucs_status_t uct_dc_mlx5_iface_flush_dcis(uct_dc_mlx5_iface_t *iface)
{
    ...
    for (i = 0; i < iface->tx.ndci * iface->tx.num_dci_pools *iface->gp; i++) {
                           ...
                           const uct_iface_config_t *tl_config)
{
    ...
    dcis_num = self->tx.ndci * self->tx.num_dci_pools * self->gp +
               UCT_DC_MLX5_KEEPALIVE_NUM_DCIS;
    self->tx.dcis = ucs_calloc(dcis_num, sizeof(*self->tx.dcis), "dcis");
    ...
    ucs_debug("dc iface %p: using '%s' policy with %d dcis and %d cqes, "
              "dct[0]: 0x%x, dct[1]: 0x%x",
              ...
              self->gp == 1 ? 0 : self->rx.dct[1].qp_num);
