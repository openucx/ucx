...
size_t uct_ib_mlx5_ext_max_get_sgl_zcopy_count(void)
{
    ...
    ucs_list_for_each(plugin, &uct_ib_mlx5_ext_plugins, list) {
        if (ucs_unlikely(uct_ib_mlx5_ext_is_unsupported_op(
                    (const void*)plugin->ops.max_get_sgl_zcopy_count))) {
            continue;
        }

        return plugin->ops.max_get_sgl_zcopy_count();
    }
...
ucs_status_t
uct_ib_mlx5_ext_ep_get_sgl_zcopy(uct_ep_h ep, void * const *buffers,
                                 const size_t *lengths, uct_mem_h const *memhs,
                                 const uint64_t *remote_addrs,
                                 uct_rkey_t const *rkeys, const size_t *counts,
                                 const size_t *strides, size_t count,
                                 uct_completion_t *comp)
{
    ...
    ucs_list_for_each(plugin, &uct_ib_mlx5_ext_plugins, list) {
        if (ucs_unlikely(uct_ib_mlx5_ext_is_unsupported_op(
                    (const void*)plugin->ops.ep_get_sgl_zcopy))) {
            continue;
        }

        return plugin->ops.ep_get_sgl_zcopy(ep, buffers, lengths, memhs,
                                            remote_addrs, rkeys, counts,
                                            strides, count, comp);
    }
...
ucs_status_t uct_ib_mlx5_ext_register(const uct_ib_mlx5_ext_ops_t *ops)
{
    ...
    ucs_debug("ib mlx5 ext: registered plugin name=%s iface_query=%s "
              "put_sgl_zcopy=%s get_sgl_zcopy=%s outstanding_purge=%s "
              "(total=%u)",
              plugin->ops.name,
