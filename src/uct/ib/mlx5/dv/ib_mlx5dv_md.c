                              ...
                              uint32_t *mkey)
{
    ...
    for (i = 0; i < ksm_data->mr_num; i++) {
        UCT_IB_MLX5DV_SET64(klm, klm, address, (uintptr_t)ksm_data->mrs[i]->addr);
    ...
    status = uct_ib_mlx5_devx_reg_ksm(md, atomic,
                                      (uintptr_t)ksm_data->mrs[0]->addr + off,
                                     ...
                                     uint32_t *mkey)
{
    ...
    ksm_address = ucs_align_down_pow2(mr_address, UCT_IB_MD_MAX_MR_SIZE);
    ksm_length  = ucs_align_up(mr->super.ib->length + mr_address - ksm_address,
                        ...
                        uct_ib_mlx5_devx_mem_t *memh)
{
    ...
    do {
        status = uct_ib_mlx5_devx_reg_ksm_data_contig(
                md, &memh->mrs[UCT_IB_MR_DEFAULT], 0, 0, "indirect key",
                        ...
                        uct_ib_mlx5_devx_mem_t *memh)
{
    ...
    status = uct_ib_mlx5_devx_reg_ksm_data_contig(
            md, mr, uct_ib_md_atomic_offset(mr_id), is_atomic, "atomic key",
            &memh->atomic_dvmr, &memh->atomic_rkey);
    if (status != UCS_OK) {
        return status;
    }

    ucs_debug("KSM registered atomic %p..%p offset 0x%x%s on %s rkey 0x%x",
              mr->super.ib->addr,
                         ...
                         uct_mem_h *memh_p)
{
    ...
    status = uct_ib_memh_new(&md->super, length,
...
uct_ib_devx_dereg_invalidate_rkey_check(uct_ib_mlx5_md_t *md,
                                        uct_ib_mlx5_devx_mem_t *memh,
                                        uint32_t rkey, uint32_t flags_mask,
                                        uint64_t cap_mask, const char *name)
{
    ...
    if (!(md->super.cap_flags & cap_mask)) {
        ucs_error("%s: invalidate %s is not supported (rkey=0x%x)",
                                                   ...
                                                   uct_ib_mlx5_devx_mem_t *memh)
{
    ...
    UCT_IB_MLX5DV_SET(mkc, mkc, pd, uct_ib_mlx5_devx_md_get_pdn(md));
    UCT_IB_MLX5DV_SET(mkc, mkc, mkey_7_0, md->mkey_tag);
    UCT_IB_MLX5DV_SET64(mkc, mkc, start_addr, (uintptr_t)address);
    UCT_IB_MLX5DV_SET64(mkc, mkc, len, length);
                           ...
                           void *mkey_buffer)
{
    ...
    if (ucs_unlikely(memh->atomic_dvmr == NULL) &&
        ...
        ucs_assertv(memh->atomic_rkey != UCT_IB_INVALID_MKEY, "dev=%s memh=%p",
                                         ...
                                         uct_mem_h *memh_p)
{
    ...
    status = uct_ib_memh_new(&md->super, 0, 0, sizeof(*memh), 0, &ib_memh);
    if (status != UCS_OK) {
        goto err;
    }
    ...
    mkey = (UCT_IB_MLX5DV_GET(create_alias_obj_out, out, hdr.obj_id) << 8) |
           md->mkey_tag;
    memh->super.lkey   = mkey;
    memh->super.rkey   = mkey;
    memh->super.flags |= UCT_IB_MEM_IMPORTED;
    *memh_p            = ib_memh;
    return UCS_OK;
