...
uct_ib_mlx5_get_cqe(uct_ib_mlx5dv_cq_t *cq,  unsigned index)
{
    return cq->dv.buf + ((index & (cq->dv.cqe_cnt - 1)) << cq->cqe_size_log);
}

static UCS_F_ALWAYS_INLINE struct mlx5_cqe64*
uct_ib_mlx5_poll_cq(uct_ib_iface_t *iface, uct_ib_mlx5dv_cq_t *cq)
{
    ...
    if (ucs_unlikely((op_own & MLX5_CQE_OWNER_MASK) == !(index & cq->dv.cqe_cnt))) {
        return NULL;
