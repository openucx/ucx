...
#define UCT_RC_MLX5_TXWQ_TX_ID_BITS 24
#define UCT_RC_MLX5_TXWQ_TX_ID_MASK UCS_MASK(UCT_RC_MLX5_TXWQ_TX_ID_BITS)


static UCS_F_ALWAYS_INLINE void
uct_rc_mlx5_txwq_set_wqe_priv(uct_ib_mlx5_txwq_t *txwq, uint16_t pi,
                              uint16_t num_bb)
{
    unsigned index                 = pi % txwq->bb_max;
    uct_ib_mlx5_txwq_priv_t *entry = &txwq->wqe_priv[index];
    ...
    entry->tx_id  = txwq->next_tx_id & UCT_RC_MLX5_TXWQ_TX_ID_MASK;
    entry->pi     = pi;
    entry->num_bb = num_bb;

    txwq->next_tx_id = (txwq->next_tx_id + 1) &
                       UCT_RC_MLX5_TXWQ_TX_ID_MASK;
}
                             ...
                             uct_ib_log_sge_t *log_sge)
{
    ...
    if (txwq->wqe_priv != NULL) {
        uct_rc_mlx5_txwq_set_wqe_priv(txwq, pi, num_bb);
    }
