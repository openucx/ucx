...
UCS_CLASS_INIT_FUNC(uct_rc_mlx5_base_ep_t, const uct_ep_params_t *params)
{
    ...
    self->tx.wq.next_tx_id = 0;
    self->tx.wq.wqe_priv   = ucs_calloc(self->tx.wq.bb_max,
                                        sizeof(*self->tx.wq.wqe_priv),
                                        "rc_mlx5_txwq_priv");
    if (self->tx.wq.wqe_priv == NULL) {
