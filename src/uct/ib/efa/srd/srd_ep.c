...
void uct_srd_ep_send_op_completion(uct_srd_send_op_t *send_op)
{
    ...
    send_op->completion(send_op, UCS_OK);
    ucs_mpool_put(send_op);
}

static void
uct_srd_send_op_empty_completion(uct_srd_send_op_t UCS_V_UNUSED *send_op,
...
static void uct_srd_ep_send_op_purge(uct_srd_ep_t *ep)
{
    ...
    ucs_list_for_each(send_op, &iface->tx.outstanding_list, list) {
        if (send_op->ep == ep) {
             ...
             */
            send_op->completion(send_op, UCS_ERR_CANCELED);
            send_op->completion = uct_srd_send_op_empty_completion;
            send_op->ep         = NULL;
            if (--ep->inflight == 0) {
                break;
            }
...
uct_srd_post_complete(uct_srd_iface_t *iface, uct_srd_ep_t *ep)
{
    iface->tx.available--;
    ep->inflight++;
}
                  ...
                  struct ibv_send_wr *wr, unsigned send_flags)
{
    ...
    uct_srd_post_complete(iface, ep);
    ep->psn++;
}
...
uct_srd_ep_get_send_desc(uct_srd_iface_t *iface, uct_srd_ep_t *ep)
{
    ...
    send_desc->super.completion = uct_srd_send_op_empty_completion;
    return send_desc;
}
...
static void
uct_srd_send_op_user_completion(uct_srd_send_op_t *send_op, ucs_status_t status)
{
...
static void uct_srd_send_desc_tx(uct_srd_iface_t *iface, uct_srd_ep_t *ep,
                                 uct_srd_send_desc_t *desc, size_t length)
{
    ...
    uct_srd_post_send(iface, ep, &iface->tx.wr_desc, 0);

    iface->tx.send_desc = NULL;
                            ...
                            unsigned flags)
{
    ...
    desc->super.completion = uct_srd_send_op_empty_completion;

    uct_srd_hdr_set(ep, desc->hdr, id);
    iface->tx.wr_desc.num_sge = 1;
    uct_srd_send_desc_tx(iface, ep, desc, length);

    UCT_TL_EP_STAT_OP(&ep->super, AM, BCOPY, length);
    return length;
}

static void uct_srd_log_rma(uct_srd_iface_t *iface, uct_srd_ep_t *ep,
                            uct_srd_send_op_t *send_op,
                            const struct ibv_sge *sge, size_t num_sge,
                            uint64_t remote_addr, uct_rkey_t rkey)
{
    struct ibv_send_wr wr = {
        .wr_id               = (uintptr_t)send_op,
        .send_flags          = 0,
        .opcode              = IBV_WR_RDMA_READ,
        .wr.rdma.remote_addr = remote_addr,
        .wr.rdma.rkey        = rkey,
        .sg_list             = (struct ibv_sge*)sge,
        .num_sge             = num_sge,
        .next                = NULL
    };

    __uct_ib_log_post_send_one(__FILE__, __LINE__, __func__, &iface->super,
                               iface->qp, &wr, 2, NULL, ep->ah, ep->dest_qpn);
}

static UCS_F_ALWAYS_INLINE ucs_status_t
uct_srd_post_rma(uct_srd_iface_t *iface, uct_srd_ep_t *ep,
                 ...
                 size_t num_sge, uint64_t remote_addr, uct_rkey_t rkey)
{
    ...
    if (ucs_log_is_enabled(UCS_LOG_LEVEL_TRACE_DATA)) {
        uct_srd_log_rma(iface, ep, send_op, sge, num_sge, remote_addr, rkey);
    }
                                  ...
                                  uct_rkey_t rkey, uct_completion_t *comp)
{
    ...
    send_op->completion = uct_srd_send_op_user_completion;

    num_sge = uct_ib_verbs_sge_fill_iov(iface->tx.sge, iov, iovcnt);
    status  = uct_srd_post_rma(iface, ep, send_op, iface->tx.sge, num_sge,
                                                 ...
                                                 ucs_status_t status)
{
    ...
    uct_srd_send_op_user_completion(send_op, status);
}
                                  ...
                                  uct_rkey_t rkey, uct_completion_t *comp)
{
    ...
    desc->super.completion = uct_srd_send_op_get_bcopy_completion;
    ...
    status = uct_srd_post_rma(iface, ep, &desc->super, iface->tx.sge, 1,
                              remote_addr, uct_ib_md_direct_rkey(rkey));
    if (!UCS_STATUS_IS_ERR(status)) {
