                           ...
                           uint64_t signal, int max_log_sge)
{
    ...
    uct_ib_log_exp_post_send(&iface->super.super, ep->qp, wr, max_log_sge,
                             (wr->exp_opcode == IBV_EXP_WR_SEND) ?
                             uct_rc_ep_packet_dump : NULL);

    ret = ibv_exp_post_send(ep->qp, wr, &bad_wr);
    if (ret != 0) {
                                           ...
                                           const uct_ep_addr_t *ep_addr)
{
    ...
    status = uct_rc_iface_qp_connect(iface, ep->qp, qp_num, &ah_attr);
    if (status != UCS_OK) {
        return status;
    }
...
UCS_CLASS_INIT_FUNC(uct_rc_verbs_ep_t, const uct_ep_params_t *params)
{
    ...
    self->super.txqp.qp_num = self->qp->qp_num;
