...
#define UCT_RC_VERBS_IFACE_FOREACH_TXWQE(_iface, _i, _wc, _num_wcs) \
      status = uct_ib_poll_cq((_iface)->super.cq[UCT_IB_TX], &_num_wcs, _wc); \
      if (status != UCS_OK) { \
          return 0; \
      } \
...
uct_rc_verbs_iface_poll_rx_common(uct_rc_iface_t *iface)
{
    ...
    status = uct_ib_poll_cq(iface->super.cq[UCT_IB_RX], &num_wcs, wc);
    if (status != UCS_OK) {
        num_wcs = 0;
        goto out;
    }
                              ...
                              uct_rc_iface_t *rc_iface)
{
    ...
    num_wcs = ibv_exp_poll_cq(rc_iface->super.cq[UCT_IB_RX], max_wcs, wc,
                              sizeof(wc[0]));
    if (num_wcs <= 0) {
