...
ucs_status_t uct_rc_mlx5_ep_get_address(uct_ep_h tl_ep, uct_ep_addr_t *addr)
{
    ...
    if (UCT_RC_MLX5_TM_ENABLED(iface)) {
        uct_ib_pack_uint24(rc_addr->tm_qp_num, ep->tm_qp.verbs.qp->qp_num);
    }
                                          ...
                                          const uct_ep_addr_t *ep_addr)
{
    ...
    if (UCT_RC_MLX5_TM_ENABLED(iface)) {
        ...
        status = uct_rc_iface_qp_connect(&iface->super, ep->tm_qp.verbs.qp,
    ...
    status = uct_rc_iface_qp_connect(&iface->super, ep->tx.qp.verbs.qp, qp_num, &ah_attr);
    if (status != UCS_OK) {
        return status;
    }
...
UCS_CLASS_INIT_FUNC(uct_rc_mlx5_ep_t, const uct_ep_params_t *params)
{
    ...
    status = uct_rc_iface_qp_create(&iface->super, &self->tx.qp.verbs.qp,
                                    &cap, iface->super.config.tx_qp_len);
    if (status != UCS_OK) {
        return status;
    }

    self->super.txqp.qp_num = self->tx.qp.verbs.qp->qp_num;

    status = uct_rc_iface_qp_init(&iface->super, self->tx.qp.verbs.qp);
    if (status != UCS_OK) {
        goto err;
    }
    ...
    self->tx.qp.type = UCT_IB_MLX5_QP_TYPE_VERBS;
...
err:
    ibv_destroy_qp(self->tx.qp.verbs.qp);
    return status;
}
...
static UCS_CLASS_CLEANUP_FUNC(uct_rc_mlx5_ep_t)
{
    ...
    uct_ib_mlx5_txwq_cleanup(&self->tx.qp, &self->tx.wq);
    uct_rc_mlx5_ep_clean_qp(self, self->tx.qp.verbs.qp);
#if IBV_HW_TM
    if (UCT_RC_MLX5_TM_ENABLED(iface)) {
