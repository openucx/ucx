...
public class UcpEndpointTest extends UcxTest {
    ...
    public void testActiveMessages() throws Exception {
        ...
        // Test rndv flow
        worker1.setAmRecvHandler(0, new UcpAmRecvCallback() {
            @Override
            public int onReceive(long headerAddress, long headerSize,
                                 UcpAmData amData, UcpEndpoint replyEp) {
                assertFalse(amData.isReceived());
                try {
                    assertEquals(headerString,
                        UcxUtils.getByteBufferView(headerAddress, (int)headerSize)
        ...
        // Test eager flow
        worker1.setAmRecvHandler(1, new UcpAmRecvCallback() {
            @Override
            public int onReceive(long headerAddress, long headerSize, UcpAmData amData,
                                 UcpEndpoint replyEp) {
                assertTrue(amData.isReceived());
                try {
        ...
        // Reset AM callback
        worker1.setAmRecvHandler(0, null);
        worker1.setAmRecvHandler(1, null);
