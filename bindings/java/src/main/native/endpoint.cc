...
{
    ucp_rkey_h rkey;
    void *rkey_buffer = (uint8_t *)env->GetDirectBufferAddress(rkey_buf) + offset;

    ucs_status_t status = ucp_ep_rkey_unpack((ucp_ep_h) ep_ptr, rkey_buffer, &rkey);
    if (status != UCS_OK) {
        JNU_ThrowExceptionByStatus(env, status);
    }
...
Java_org_ucx_jucx_ucp_UcpEndpoint_putNonBlockingNative(JNIEnv *env, jclass cls,
                                                       ...
                                                       jobject callback)
{
    void *src_addr =  (uint8_t *)env->GetDirectBufferAddress(src_buf) + offset;
    ucs_status_ptr_t request = ucp_put_nb((ucp_ep_h) ep_ptr, src_addr, size,
                                          dst_addr, (ucp_rkey_h)rkey_ptr, jucx_request_callback);

    ucs_trace_req("JUCX: put_nb request %p to %s, of size: %zu, raddr: %zu",
                  request, ucp_ep_peer_name((ucp_ep_h)ep_ptr), size, dst_addr);
...
Java_org_ucx_jucx_ucp_UcpEndpoint_getNonBlockingNative(JNIEnv *env, jclass cls,
                                                       ...
                                                       jobject callback)
{
    void *result_address = (uint8_t *)env->GetDirectBufferAddress(dst_buf) + offset;

    ucs_status_ptr_t request = ucp_get_nb((ucp_ep_h)ep_ptr, result_address, size,
    ...
    ucs_trace_req("JUCX: get_nb request %p to %s, raddr: %zu, size: %zu, result address: %p",
                  request, ucp_ep_peer_name((ucp_ep_h)ep_ptr), address, size, result_address);
    return process_request(request, callback);
}
...
{
    void *buffer = (uint8_t *)env->GetDirectBufferAddress(send_buf) + offset;
    ucs_status_ptr_t request = ucp_tag_send_nb((ucp_ep_h)ep_ptr, buffer, size,
                                               ucp_dt_make_contig(1), tag, jucx_request_callback);

    ucs_trace_req("JUCX: send_nb request %p to %s, offset: %zu, size: %zu, tag: %ld",
                  request, ucp_ep_peer_name((ucp_ep_h)ep_ptr), offset, size, tag);
    return process_request(request, callback);
}
