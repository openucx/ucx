...
/*
 * UCP multi-GPU client - server example utility
 * -----------------------------------------------
 *
 * Server side:
 *
 ...
 * Client side:
 *
 *    ./ucp_client_server_multi_dev -a <server-ip>
 *
 * Notes:
 *
...
{
    if (is_server) {
        printf("Server: iteration #%d, server GPU %d, client GPU %d\n",
               (current_iter + 1), sdev, cdev);
        printf("UCX data message was received\n");
        printf("\n----- UCP TEST SUCCESS -------\n\n");
    } else {
        printf("Client: iteration #%d, server GPU %d, client GPU %d\n",
               (current_iter + 1), sdev, cdev);
        printf("\n------------------------------\n\n");
    }
...
int main(int argc, char **argv)
{
    ...
    if (test_mem_type != UCS_MEMORY_TYPE_CUDA &&
        ...
        fprintf(stderr, "test memory type must be cuda or cuda-managed\n");
        ret = -1;
        goto err;
    }
    ...
    for (dev_id = 0; dev_id < dev_count; dev_id++) {
         ...
         * Server needs it to receive the active messages
         * Client needs it to receive the FIN message */
