...
int parse_cmd(int argc, char * const argv[], char **server_ip);
...
int main(int argc, char **argv)
{
	...
	status = parse_cmd(argc, argv, &client_target_ip);
  ...
  CHKERR_JUMP(status != UCS_OK, "ucp_worker_create\n", err_cleanup);
        ...
        //printf("my port is: %d\n", server_port);
        //oob_sock = client_connect(client_target_ip, server_port);
      ...
      /* Send client UCX address to server */
    	...
    	set_connect_addr(client_target_ip, &connect_addr);
...
int parse_cmd(int argc, char * const argv[], char **server_ip)
{
    ...
    while ((c = getopt(argc, argv, "wfben:c:p:s:h")) != -1) {
        switch (c) {
        case 'w':
        ...
        case 'h':
        default:
            fprintf(stderr, "Usage: ucp_rdma_client [parameters]\n");
            fprintf(stderr, "UCP get client example utility\n");
            fprintf(stderr, "\nParameters are:\n");
            ...
            fprintf(stderr, "  -n IP address of the server (required for client)\n");
            fprintf(stderr, "  -c number of connections from the client (default: 1)\n");
            fprintf(stderr, "  -m size of RDMA reads (default: 1024)\n");
    ...
    fprintf(stderr, "INFO: UCP_RDMA_CLIENT mode = %d server = %s port = %d\n",
            ucp_test_mode, *server_ip, server_port);
