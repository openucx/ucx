...
void set_listen_addr(char* address_str , struct sockaddr_in *listen_addr);

void* run_server(void* thread_info);

void* run_connection_manager(void* info);

int parse_cmd(int argc, char * const argv[], int *workers_num , char** server_ip);
...
int main(int argc, char **argv)
{
	...
	status = parse_cmd(argc, argv, &workers_num , &server_ip);
	...
	rc = pthread_create(&connection_manager_thread , NULL , &run_connection_manager , NULL);
        if(rc){
...
void* run_connection_manager(void* info){
	...
	status = ucp_worker_create(ucp_context, &worker_params, &cm_worker);
	CHKERR_JUMP(status != UCS_OK, "ucp_worker_create\n", err_cleanup);

	//status = ucp_worker_get_address(cm_worker, &local_addr, &local_addr_len);
	//CHKERR_JUMP(status != UCS_OK, "ucp_worker_get_address\n", err_worker);

	struct sockaddr_in listen_addr;
	ucp_listener_params_t params;
	ucp_listener_h listener;
	set_listen_addr(server_ip , &listen_addr);
