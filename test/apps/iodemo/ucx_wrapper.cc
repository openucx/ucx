...
/*
 * Asynchronous request private data
 */
struct ucx_request {
    UcxCallback              *callback;     /* callback to call when completed */
    UcxConnection            *conn;         /* connection this request used */
    bool                     completed;     /* completion flag */
    ucs_status_t             status;        /* completion status */
    uint32_t                 iomsg_conn_id; /* connection id for received IO message */
    size_t                   recv_length;   /* actual length of received data */
    ucx_request_list_iter_t  pos;           /* position in all-requests list */
};
                                     ...
                                     ucp_tag_recv_info *info)
{
    ...
    r->completed     = true;
    r->iomsg_conn_id = (info->sender_tag & ~IOMSG_TAG) >> 32;
...
// Check if the receive request for IO message is completed
void UcxContext::progress_io_message()
{
    ...
    uint32_t conn_id = _iomsg_recv_request->iomsg_conn_id;
    ...
    // Recycle the IO message receive request
...
UcxConnection::~UcxConnection()
{
    ...
    if (_close_request) {
        // Wait until endpoint is really destroyed so no callbacks will be
        // called for it
        _context.wait_completion(_close_request);
    }

    // Wait until all outstanding requests are completed
...
void UcxConnection::cancel_all()
{
    ...
    unsigned count = 0;
    for (ucx_request_list_iter_t iter = requests.begin(); iter != requests.end();
         ++iter) {
...
bool UcxConnection::connect_common(ucp_ep_params_t ep_params)
{
    ...
    // add the connection to the map on the context
                                    ...
                                    UcxCallback* callback)
{
    ...
    if (ptr_status == NULL) {
        // Operation completed immediately, with success
        (*callback)(UCS_OK);
    } else if (UCS_PTR_IS_ERR(ptr_status)) {
        // Operation completed immediately, with failure
        status = UCS_PTR_STATUS(ptr_status);
        UCX_CONN_LOG << what << "failed with status: "
                     << ucs_status_string(status);
        (*callback)(status);
    } else {
        // Operation started and will complete asynchronously
        ...
        if (r->completed) {
            ...
            // Will be completed by callback later
            r->callback = callback;
            r->conn     = this;
            request_started(r);
        }
