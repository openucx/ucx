...
public:
    ...
    // Wait until all responses arrive, or timeout/failure occurs
    bool wait_for_responses(long max_outstanding) {
        double current_time, start_time = 0.0;
        ...
        while ((_num_sent - _num_completed > max_outstanding) && !_error_flag) {
            ...
            // Make some polling loops without checking timeout, since checking
            // the current time has some overhead
            if (count < 1000) {
                progress();
                ++count;
                continue;
            ...
            // Check for timeout
            if ((current_time - start_time) > _test_opts.client_timeout) {
    ...
    bool run() {
        ...
        while ((iteration < opts().iter_count) && !_error_flag) {
            ...
            // Report performance every second, measure time every 10
            // iterations
