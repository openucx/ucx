...
UCS_TEST_P(test_ucp_wireup_1sided, multi_ep_1sided) {
    ...
    for (unsigned i = 0; i < count; ++i) {
        connect(sender(), receiver(), get_ep_params(), i);
    }
...
void test_ucp_wireup_2sided::test_connect_loopback(bool delay_before_connect,
    ...
    for (int i = 0; i < 5; ++i) {
        int base_index = i * 2;
        connect(sender(), sender(), params, base_index);
        ...
        connect(sender(), sender(), params, base_index + 1);
...
UCS_TEST_P(test_ucp_wireup_2sided, multi_ep_2sided) {
    ...
    for (unsigned j = 0; j < 4; ++j) {
        ...
        for (unsigned i = 0; i < count; ++i) {
            ...
            connect(sender(), receiver(), get_ep_params(), ep_idx);
            if (!is_loopback()) {
                connect(receiver(), sender(), get_ep_params(), ep_idx);
            }
