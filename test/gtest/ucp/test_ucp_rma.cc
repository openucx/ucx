...
UCS_TEST_P(test_ucp_rma_sgl, put_am_pending)
{
    ...
    static const char am_msg[]  = "am";
    static const unsigned am_id = 1;

    auto am_cb = [](void *, const void *, size_t, void *, size_t,
                    const ucp_am_recv_param_t *) -> ucs_status_t {
        return UCS_OK;
    };
    ucp_am_handler_param_t am_handler = {};
    am_handler.field_mask = UCP_AM_HANDLER_PARAM_FIELD_ID |
    ...
    auto wait_req = [&](void *req) {
        if (req == NULL) {
            return;
        }

        if (!UCS_PTR_IS_PTR(req)) {
            EXPECT_UCS_OK(UCS_PTR_STATUS(req));
            return;
        }
        ...
        EXPECT_UCS_OK(ucp_request_check_status(req));
        ucp_request_free(req);
    };

    for (size_t c = 0; c < ucs_static_array_size(cases); c++) {
        const size_t size        = cases[c][0];
        const size_t count       = cases[c][1];
        const size_t repeat      = cases[c][2];
        const size_t num_threads = cases[c][3];

        sgl_ctx ctx;
        init_sgl_ctx(ctx, count, size);

        ucp_dt_local_sgl_t local   = make_local_sgl(
                ctx, LOCAL_MASK_DEFAULT | UCP_DT_LOCAL_SGL_FIELD_MEMHS);
        ucp_dt_remote_sgl_t remote = make_remote_sgl(ctx, REMOTE_MASK_DEFAULT);
        ucp_request_param_t param  = make_sgl_param(&remote, count);
        ...
        for (size_t r = 0; r < repeat; r++) {
            ...
            for (size_t t = 0; t < num_threads; t++) {
                threads.emplace_back([&]() {
                    void *put_req = sgl_op_nbx(SGL_OP_PUT, &local, count,
                                               UCP_REMOTE_ADDR_INVALID,
                                               UCP_RKEY_INVALID, &param);
                    void *flush_req = sender().flush_ep_nb();

                    ucp_request_param_t am_param = {};
                    am_param.op_attr_mask        = UCP_OP_ATTR_FIELD_FLAGS;
                    am_param.flags               = UCP_AM_SEND_FLAG_EAGER;
                    void *am_req = ucp_am_send_nbx(sender().ep(), am_id, NULL,
                                                   0, am_msg, sizeof(am_msg),
                                                   &am_param);
