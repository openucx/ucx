        ...
        bool force_invalidate_rma)
{
    ...
    int no_invalidate_rma = 0;
    int invalidate_rma    = 0;
    ...
    if (force_invalidate_rma) {
        ASSERT_TRUE(no_invalidate_rma == 0);
    } else {
        ASSERT_TRUE(!has_transport("tcp") || (no_invalidate_rma != 0));
        ASSERT_TRUE(!has_transport("rc_x") || (invalidate_rma != 0));
    }
}
