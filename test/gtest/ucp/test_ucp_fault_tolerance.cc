...
protected:
    ...
    static void shuffle_lanes(std::vector<ucp_lane_index_t> &lanes, const std::string &lane_type) {
        if (lanes.size() < 2) {
            UCS_TEST_SKIP_R("At least 2 " + lane_type + "s are required, but only " + std::to_string(lanes.size()) +
                            " available");
        }
    ...
    void test_am_with_injected_failure(failure_side_t failure_side, bool flush_after = false) {
        ...
        ucp_lane_index_t *am_bw_lane_idx;
        ucs_carray_for_each(am_bw_lane_idx, ucp_ep_config(sender().ep(0, INJECTED_EP_INDEX))->key.am_bw_lanes,
                            UCP_MAX_LANES) {
        ...
        ucs_status_t status = do_am_send_and_wait(sender().ep(0, INJECTED_EP_INDEX), am_msg_size(), flush_after);
