           ...
           "IB_NUM_PATHS?=1", "MAX_RMA_RAILS=2", "ZCOPY_THRESH=0")
{
    ...
    /* Two rounding stages in multi-rail length calculation may cause each rail
     * to differ from the ideal value by up to 1 byte. Use 2-byte tolerance
     * (1 byte per rail) to avoid false failures due to rounding effects.
     */
    constexpr double byte_tolerance = 2.0;
