...
private:
    ...
    {
        uct_iface_params_t copy_params   = *params;
        copy_params.mode.device.dev_name = m_self->m_real_dev_name.c_str();

        UCS_MOCK_ORIG_FUNC(m_self->m_mock, &m_self->m_tl->iface_open, md,
                           worker, &copy_params, config, iface_p);
