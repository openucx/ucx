...
void uct_test::entity::mem_alloc(void **address_p, size_t *length_p,
    ...
    *method = (pd_attr.cap.flags & UCT_PD_FLAG_ALLOC) ?
                    UCT_ALLOC_METHOD_PD : UCT_ALLOC_METHOD_MMAP;
    status = uct_pd_mem_alloc(m_iface->pd, *method, alignement, "test", length_p,
                              address_p, memh_p);
    ASSERT_UCS_OK(status);
...
void uct_test::entity::mem_free(void *address, size_t length, uct_mem_h memh,
                                uct_alloc_method_t method,
                                const uct_rkey_bundle_t& rkey) const {
    ucs_status_t status;
    uct_rkey_release(m_ucth, const_cast<uct_rkey_bundle_t*>(&rkey));
    status = uct_pd_mem_free(m_iface->pd, method, address, length, memh);
    ASSERT_UCS_OK(status);
}
...
uct_test::mapped_buffer::mapped_buffer(size_t size, size_t alignment,
    ...
    m_alloc_size(size), m_entity(entity)
{
    if (size > 0) {
        m_entity.mem_alloc(&m_buf, &m_alloc_size, alignment, &m_memh, &m_method,
                           &m_rkey);
        ...
        m_method    = UCT_ALLOC_METHOD_LAST;
        m_rkey.rkey = UCT_INVALID_MEM_HANDLE;
        m_rkey.type = NULL;
    }
    pattern_fill(seed);
}
...
uct_test::mapped_buffer::~mapped_buffer() {
    if (m_memh != UCT_INVALID_MEM_HANDLE) {
        m_entity.mem_free(m_buf, m_alloc_size, m_memh, m_method, m_rkey);
    }
}
