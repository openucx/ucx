 ...
 * same base address. The cache must detect the allocation-id mismatch on the
...
/*
 * A peer can exit and a new process can reuse its PID. alloc_id is a
 * per-process counter, so the restarted peer's first allocation can report the
 * same (pid, address, alloc_id, length) as the dead peer's - every field the
 * cache compares on a hit. Two distinct allocations are forced to agree on all
 * four; the cache must still not hand back the first mapping.
 */
UCS_TEST_F(test_ze_ipc_cache, pid_reuse_is_remapped) {
    ...
    /* a restarted peer on a reused PID: same VA, same length, and its
     * per-process allocation counter has handed out the same id again. Only the
     * process creation time differs, which is the whole point */
    key_b.address          = key_a.address;
    key_b.alloc_id         = key_a.alloc_id;
    key_b.proc_create_time = key_a.proc_create_time + 1;
    ASSERT_EQ(key_a.pid, key_b.pid);
    ASSERT_EQ(key_a.length, key_b.length);
    ASSERT_NE(0u, key_a.proc_create_time);
    ...
    EXPECT_NE(mapped_a, mapped_b)
            << "cache returned the dead peer's mapping after PID reuse";
