...
#include "uct_entity.h"

#include <ucs/gtest/test_helpers.h>


std::vector<uct_resource_desc_t> uct_entity::enum_resources(const std::string& tl_name) {
...
uct_entity::uct_entity(const uct_resource_desc_t& resource) {
    ucs_status_t status;
...
uct_entity::~uct_entity() {
    uct_ep_destroy(m_ep);
    uct_iface_close(m_iface);
    uct_cleanup(m_ucth);
}

void uct_entity::connect(const uct_entity& other) {
    ucs_status_t status;
...
uct_rkey_bundle_t uct_entity::mem_map(void *address, size_t length, uct_lkey_t *lkey_p) const {
    ucs_status_t status;
    void *rkey_buffer;
...
void uct_entity::mem_unmap(uct_lkey_t lkey, const uct_rkey_bundle_t& rkey) const {
    ucs_status_t status;
...
uct_ep_h uct_entity::ep() const {
    return m_ep;
}

void uct_entity::flush() const {
    ucs_status_t status;
    do {
        uct_progress(m_ucth);
