...
protected:
    ...
    void
    prepare_is_connected(uct_ep_is_connected_params_t *params, entity **e1_p)
    {
        ...
        ASSERT_UCS_OK(
                uct_iface_get_address(m_receiver->iface(),
                                      (uct_iface_addr_t*)iface_addr.data()));
        ASSERT_UCS_OK(uct_iface_get_device_address(
                m_receiver->iface(), (uct_device_addr_t*)dev_addr.data()));
...
UCS_TEST_P(test_uct_ep, is_connected)
{
    ...
    prepare_is_connected(&params, &e1);
    EXPECT_TRUE(uct_ep_is_connected(m_sender->ep(0), &params));
}

UCS_TEST_SKIP_COND_P(test_uct_ep, not_connected,
                     has_cma() || has_transport("knem"))
{
    uct_ep_is_connected_params_t params;
    entity *e1;
