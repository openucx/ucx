...
UCS_TEST_SKIP_COND_P(test_rc, stress_iface_ops,
    ...
    const std::string name = wrap_config_str("TX_CQ_LEN");
    int cq_len             = 16;

    if (UCS_OK != uct_config_modify(m_iface_config, name.c_str(),
...
protected:
    virtual void init() {
        ...
        const std::string name1 = wrap_config_str("VERBS_TX_MAX_WR");
        const std::string name2 = wrap_config_str("TX_MAX_BB");
        status1 = uct_config_modify(m_iface_config, name1.c_str(), "32");
        status2 = uct_config_modify(m_iface_config, name2.c_str(), "32");
...
public:
    ...
    test_rc_get_limit() {
        ...
        modify_config("TX_NUM_GET_BYTES", ucs::to_string(m_num_get_bytes));

        m_max_get_zcopy = 4096;
        modify_config("MAX_GET_ZCOPY", ucs::to_string(m_max_get_zcopy),
                      FAIL_IF_NOT_EXIST);

        if (!RUNNING_ON_VALGRIND) {
            /* Valgrind already has special small value for this */
            modify_config("TX_QUEUE_LEN", "32");
        }

        modify_config("TM_ENABLE", "y", SETENV_IF_NOT_EXIST);
                     ...
                     !check_caps(UCT_IFACE_FLAG_GET_ZCOPY))
{
    ...
    modify_config("MAX_GET_ZCOPY", ucs::to_string(max_ib_msg + 1));
