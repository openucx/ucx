...
protected:
    ...
    static ucs_status_t
    qp_query(struct ibv_qp*, struct mlx5dv_devx_obj*,
    ...
    static ucs_status_t
    extract(uct_ep_h, const uct_ep_outstanding_extract_params_t *params)
    {
    ...
    static void extract_cb(const uct_ep_op_info_t*, void *arg)
    {
    ...
    register_plugin(const char *name,
                    ...
                    uct_ib_mlx5_ext_qp_query_func_t qp_query_cb = NULL,
                    uct_ep_outstanding_extract_func_t extract_cb = NULL)
    {
        uct_ib_mlx5_ext_ops_t ops = {};

        ucs_strncpy_zero(ops.name, name, sizeof(ops.name));
        ops.iface_query            = iface_query_cb;
        ops.qp_query               = qp_query_cb;
        ops.ep_outstanding_extract = extract_cb;
...
UCS_TEST_P(test_uct_ib_mlx5_ext_rc, iface_query)
{
    ...
    register_plugin("token", iface_query, qp_query, extract);
...
UCS_TEST_P(test_uct_ib_mlx5_ext_rc, ep_outstanding_extract)
{
...

    {
        scoped_log_handler wrap_err(wrap_errors_logger);
        EXPECT_EQ(UCS_ERR_INVALID_PARAM,
                  uct_ep_outstanding_extract(m_e1->ep(0), &params));
    }

    register_plugin("stub",
                    (uct_ib_mlx5_ext_iface_query_func_t)
                            ucs_empty_function_return_unsupported,
                    (uct_ib_mlx5_ext_qp_query_func_t)
                            ucs_empty_function_return_unsupported,
                    (uct_ep_outstanding_extract_func_t)
                            ucs_empty_function_return_unsupported);
    register_plugin("token", iface_query, qp_query, extract);
    ...
    ASSERT_UCS_OK(uct_ep_outstanding_extract(m_e1->ep(0), &params));
