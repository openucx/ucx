...
ucs_config_field_t seat_opts_table[] = {
  ...
  {"MATERIAL", "textile",
   "Cover seat material.",
   ucs_offsetof(seat_opts_t, material),
   UCS_CONFIG_TYPE_ENUM(material_names)},
...
ucs_config_field_t engine_opts_table[] = {
  {"MODEL", "auto",
   "Engine model.",
   ucs_offsetof(engine_opts_t, model),
...
ucs_config_field_t car_opts_table[] = {
  ...
  {"BRAND", "Chevy",
   "Car brand.",
   ucs_offsetof(car_opts_t, brand),
   UCS_CONFIG_TYPE_STRING},

  {"MODEL", "Corvette",
   "Car model.",
   ucs_offsetof(car_opts_t, model),
   UCS_CONFIG_TYPE_STRING},

  {"COLOR", "red",
   "Car color.",
   ucs_offsetof(car_opts_t, color),
   UCS_CONFIG_TYPE_ENUM(color_names)},

  {"VIN", "auto",
   "Vehicle identification number.",
   ucs_offsetof(car_opts_t, vin),
   UCS_CONFIG_TYPE_ULUNITS},

  {"BW_BYTES", "1024Bs",
   "Bandwidth in bytes.",
   ucs_offsetof(car_opts_t, bw_bytes),
   UCS_CONFIG_TYPE_BW},

  {"BW_KBYTES", "1024KB/s",
   "Bandwidth in kbytes.",
   ucs_offsetof(car_opts_t, bw_kbytes),
   UCS_CONFIG_TYPE_BW},

  {"BW_MBYTES", "1024MBs",
   "Bandwidth in mbytes.",
   ucs_offsetof(car_opts_t, bw_mbytes),
   UCS_CONFIG_TYPE_BW},

  {"BW_GBYTES", "1024GBps",
   "Bandwidth in gbytes.",
   ucs_offsetof(car_opts_t, bw_gbytes),
   UCS_CONFIG_TYPE_BW},

  {"BW_TBYTES", "1024TB/s",
   "Bandwidth in tbytes.",
   ucs_offsetof(car_opts_t, bw_tbytes),
   UCS_CONFIG_TYPE_BW},

  {"BW_BITS", "1024bps",
   "Bandwidth in bits.",
   ucs_offsetof(car_opts_t, bw_bits),
   UCS_CONFIG_TYPE_BW},

  {"BW_KBITS", "1024Kb/s",
   "Bandwidth in kbits.",
   ucs_offsetof(car_opts_t, bw_kbits),
   UCS_CONFIG_TYPE_BW},

  {"BW_MBITS", "1024Mbs",
   "Bandwidth in mbits.",
   ucs_offsetof(car_opts_t, bw_mbits),
   UCS_CONFIG_TYPE_BW},

  {"BW_GBITS", "1024Gbps",
   "Bandwidth in gbits.",
   ucs_offsetof(car_opts_t, bw_gbits),
   UCS_CONFIG_TYPE_BW},

  {"BW_TBITS", "1024Tbs",
   "Bandwidth in tbits.",
   ucs_offsetof(car_opts_t, bw_tbits),
   UCS_CONFIG_TYPE_BW},

  {"BW_AUTO", "auto",
   "Auto bandwidth value.",
   ucs_offsetof(car_opts_t, bw_auto),
   UCS_CONFIG_TYPE_BW},

  {"CAN_BUS_BW", "mlx5_0:1024Tbs",
   "Bandwidth in tbits of CAN-bus.",
   ucs_offsetof(car_opts_t, can_pci_bw),
   UCS_CONFIG_TYPE_BW_SPEC},

  {"AIR_CONDITIONING", "on",
   "Air conditioning mode.",
   ucs_offsetof(car_opts_t, air_conditioning),
   UCS_CONFIG_TYPE_ON_OFF},

  {"ABS", "off",
   "ABS mode.",
   ucs_offsetof(car_opts_t, abs),
   UCS_CONFIG_TYPE_ON_OFF},

  {"TRANSMISSION", "auto",
   "Transmission mode.",
   ucs_offsetof(car_opts_t, transmission),
