...
protected:
    ...
    /* Brute-force reference: is any live interval a superset of [start, end)? */
    ...
    /* Ordering by start, and the max_end augmentation. The red-black
     * invariants themselves belong to the core and are checked there. */
    static void check_node(const ucs_rbtree_node_t *rb_node)
    {
        ...
        if (rb_node->left != NULL) {
            EXPECT_LE(ucs_derived_of(rb_node->left,
                                     ucs_interval_map_node_t)->start,
                      node->start);
        }
        if (rb_node->right != NULL) {
            EXPECT_GE(ucs_derived_of(rb_node->right,
                                     ucs_interval_map_node_t)->start,
                      node->start);
        }

        EXPECT_EQ(subtree_max(rb_node), node->max_end);
    }

    void validate(size_t expected)
    {
        EXPECT_TRUE(rbtree_check::validate(&m_map.rb, expected, check_node));
